...
const OFFER = {
  ...
  relay: { endpoint: "relay.claws.company:443" },
};
...
async function main() {
  ...
  await measurePings("Relay (relay.claws.company:443)", relayClient, PING_COUNT, WARMUP_COUNT);
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const ws = new WebSocket(
      `wss://relay.claws.company/ws?serverId=latency_probe_${Date.now()}&role=client&clientId=probe_${i}`,
    );
    await new Promise<void>((resolve, reject) => {
      ws.on("open", () => {
