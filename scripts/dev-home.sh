...
seed_worktree_ottie_home() {
  local source_home="${OTTIE_DEV_SEED_HOME:-$HOME/.ottie}"
  local target_home="$1"
  ...
  if [ "${OTTIE_DEV_RESET_HOME:-0}" = "1" ]; then
    rm -rf "$target_home"
...
configure_dev_ottie_home() {
  ...
  if [ -n "$git_dir" ] && [ -n "$git_common_dir" ] && [ "$git_dir" != "$git_common_dir" ]; then
    ...
    OTTIE_HOME="$HOME/.ottie-${worktree_name}"
    seed_worktree_ottie_home "$OTTIE_HOME"
    return
  fi

  OTTIE_HOME="$(mktemp -d "${TMPDIR:-/tmp}/ottie-dev.XXXXXX")"
  trap "rm -rf '$OTTIE_HOME'" EXIT
}
