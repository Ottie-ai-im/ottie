---
name: ottie-chat
description: Use chat rooms through the Ottie CLI. Use when the user says "chat room", "room", "coordinate through chat", "shared mailbox", or wants agents to communicate asynchronously.
user-invocable: true
---

# Ottie Chat Skill

This skill teaches how to use chat rooms for agent coordination via the Ottie CLI.
...
Load the **Ottie skill** first if you need CLI guidance for launching or messaging agents.

## Rules

When using chat:

- create a room with `ottie chat create` if you need a new room
- inspect available rooms with `ottie chat ls` and `ottie chat inspect`
- post with `ottie chat post`
- read with `ottie chat read`
...
```bash
ottie chat create issue-456 --purpose "Coordinate implementation and review"
```
...
```bash
ottie chat inspect issue-456
```

### Post a message

```bash
ottie chat post issue-456 "I traced the failure to relay auth. Investigating config loading now."
```

With a reply:

```bash
ottie chat post issue-456 "I can take that next." --reply-to msg-001
```

With a direct mention:

```bash
ottie chat post issue-456 "@<agent-id> Can you verify the relay path next?"
```

With a room-wide mention:

```bash
ottie chat post issue-456 "@everyone Check the latest status update and reply with blockers."
```

### Read recent messages

```bash
ottie chat read issue-456 --limit 10
```

### Filter reads

```bash
ottie chat read issue-456 --agent <agent-id>
ottie chat read issue-456 --since 5m
ottie chat read issue-456 --since 2026-03-24T10:00:00Z
```

### Wait for new messages

```bash
ottie chat wait issue-456 --timeout 60s
```
...
- your own agent ID is available via `$OTTIE_AGENT_ID`
...
2. Create the room with `ottie chat create` if needed
