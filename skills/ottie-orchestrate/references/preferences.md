...
The orchestrator reads user preferences from `~/.ottie/orchestrate.json` at startup. If the file doesn't exist, use the defaults below.
...
Maps role categories to `<agent-type>/<model>` strings. These map directly to the Ottie **create agent** tool parameters:
