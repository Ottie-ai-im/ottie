...
Use the Ottie **create agent** tool with `background: true` and `notifyOnFinish: true`:
...
Wait for all researchers to complete (you'll be notified). Use the Ottie **get agent activity** tool to read their findings. Synthesize into a research summary that feeds the planning phase.
