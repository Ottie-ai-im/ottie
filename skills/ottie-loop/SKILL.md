...
You are setting up a loop — an iterative worker/verifier cycle managed by the Ottie daemon.
...
Load the **Ottie skill** first. It contains the CLI reference for `ottie loop` and related commands.
...
```bash
ottie loop run "Check PR #42. Review CI, comments, and branch status. Fix issues as they arise." \
...
```bash
ottie loop run "Run the test suite, investigate failures, and fix the code." \
...
```bash
ottie loop run "Implement issue #456. Make incremental progress each iteration." \
  --provider codex/gpt-5.4 \
...
ottie loop inspect <id>         # Show details and iteration history
ottie loop logs <id>            # Stream logs
...
7. Run `ottie loop run` with the final arguments
