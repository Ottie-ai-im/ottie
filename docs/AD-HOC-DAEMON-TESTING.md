...
const ottieHomeRoot = await mkdtemp(path.join(os.tmpdir(), "ottie-test-"));
...
const daemon = await createOttieDaemon(
  {
...
For simpler cases, `createTestOttieDaemon` + `DaemonClient` handles temp dirs and port selection:
...
The test helper does **not** expose `providerOverrides`. Use `createOttieDaemon` directly when you need it (see quick start above).
