# Adding a New Provider to Ottie
...
ottie run --provider my-provider

# Launch with a specific model and mode
ottie run --provider my-provider --model some-model --mode default

# List running agents
ottie ls -a -g

# Check if the provider reports models
ottie models --provider my-provider
```
