...
Ottie uses **file-based JSON persistence** instead of a traditional database. All data is validated at runtime with Zod schemas and written atomically (write to temp file, then rename). There are no migrations — schemas use optional fields with defaults for forward compatibility.

All server-side stores live under `$OTTIE_HOME` (defaults to `~/.ottie`).
...
**Path:** `$OTTIE_HOME/agents/{project-dir}/{agentId}.json`
...
**Path:** `$OTTIE_HOME/schedules/{id}.json`
...
**Path:** `$OTTIE_HOME/chat/rooms.json`
...
**Path:** `$OTTIE_HOME/loops/loops.json`
...
**Path:** `$OTTIE_HOME/projects/projects.json`
...
**Path:** `$OTTIE_HOME/projects/workspaces.json`
...
**Path:** `$OTTIE_HOME/push-tokens.json`
...
**AsyncStorage key:** `ottie-drafts` (version 2)
...
**IndexedDB database:** `ottie-attachment-bytes`, object store: `attachments`
