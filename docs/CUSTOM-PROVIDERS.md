...
Each profile appears as a separate provider in the Ottie app. You can select which one to use when launching an agent.
...
The `command` array completely replaces the default command for that provider. The binary must exist on the system — Ottie checks for its availability and will mark the provider as unavailable if not found.
...
The [Agent Client Protocol (ACP)](https://agentclientprotocol.com) is an open standard for communication between editors and AI coding agents — think LSP but for AI agents. Any agent that supports ACP can be added to Ottie as a custom provider.

ACP agents communicate over JSON-RPC 2.0 on stdio. Ottie spawns the agent process and talks to it through stdin/stdout.
