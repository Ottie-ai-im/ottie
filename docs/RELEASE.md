...
If asked to "release ottie" without specifying major/minor, treat it as a patch release.
...
- `version:all:*` bumps root + syncs workspace versions and `@ottie/*` dependency versions
...
- If you are Codex and a stable release needs a changelog entry, launch a Claude agent with Ottie to draft it, then review and commit the result.

## Changelog voice

The changelog is shown on the Ottie homepage. Write it for **end users**, not developers.
...
Format: append `([#123](https://github.com/ottie/ottie/pull/123) by [@user](https://github.com/user))` at the end of each bullet. For changes spanning multiple PRs or contributors:

```markdown
- Voice mode now works on tablets with proper microphone permissions. ([#210](https://github.com/ottie/ottie/pull/210), [#215](https://github.com/ottie/ottie/pull/215) by [@alice](https://github.com/alice), [@bob](https://github.com/bob))
```

Rules:

- **Always link the PR number** as `[#N](https://github.com/ottie/ottie/pull/N)`.
...
Load the `ottie` skill and launch a **Codex 5.4** agent with a prompt like:
