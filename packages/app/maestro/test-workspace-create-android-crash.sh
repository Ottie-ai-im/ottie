...
#   OTTIE_MAESTRO_APP_ID=com.ottie.debug
#   OTTIE_MAESTRO_DIRECT_ENDPOINT=127.0.0.1:6767
#   OTTIE_MAESTRO_DAEMON_WS_URL=ws://127.0.0.1:6767/ws
#   OTTIE_MAESTRO_PROJECT_PATH=/path/to/git/repo
set -euo pipefail
...
OUT_DIR="/tmp/ottie-workspace-create-android-$(date +%s)"
SERVER_EXPORTS="$REPO_ROOT/packages/server/dist/server/server/exports.js"

export OTTIE_MAESTRO_APP_ID="${OTTIE_MAESTRO_APP_ID:-com.ottie.debug}"
export OTTIE_MAESTRO_DIRECT_ENDPOINT="${OTTIE_MAESTRO_DIRECT_ENDPOINT:-127.0.0.1:6767}"
export OTTIE_MAESTRO_DAEMON_WS_URL="${OTTIE_MAESTRO_DAEMON_WS_URL:-ws://127.0.0.1:6767/ws}"
...
render_flow() {
  ...
  perl -0pe '
    s/\$\{OTTIE_MAESTRO_APP_ID\}/$ENV{OTTIE_MAESTRO_APP_ID}/g;
    s/\$\{OTTIE_MAESTRO_DIRECT_ENDPOINT\}/$ENV{OTTIE_MAESTRO_DIRECT_ENDPOINT}/g;
    s/\$\{OTTIE_MAESTRO_PROJECT_NAME\}/$ENV{OTTIE_MAESTRO_PROJECT_NAME}/g;
  ' "$source" > "$target"
}
...
if [ ! -f "$SERVER_EXPORTS" ]; then
  ...
  echo "Run: npm run build --workspace=@ottie/server" >&2
  exit 1
fi

mkdir -p "$OUT_DIR"

if [ -z "${OTTIE_MAESTRO_PROJECT_PATH:-}" ]; then
  PROJECT_PARENT="$(mktemp -d /tmp/ottie-maestro-project-XXXXXX)"
  PROJECT_BASENAME="aaa-workspace-create-android-$(basename "$PROJECT_PARENT")"
  export OTTIE_MAESTRO_PROJECT_PATH="$PROJECT_PARENT/$PROJECT_BASENAME"
  mkdir -p "$OTTIE_MAESTRO_PROJECT_PATH"
  git -C "$OTTIE_MAESTRO_PROJECT_PATH" init >/dev/null
  git -C "$OTTIE_MAESTRO_PROJECT_PATH" checkout -b main >/dev/null 2>&1 || true
  git -C "$OTTIE_MAESTRO_PROJECT_PATH" config user.name "Ottie Maestro"
  git -C "$OTTIE_MAESTRO_PROJECT_PATH" config user.email "maestro@ottie.local"
  printf "# Workspace create Android repro\n" > "$OTTIE_MAESTRO_PROJECT_PATH/README.md"
  git -C "$OTTIE_MAESTRO_PROJECT_PATH" add README.md
  git -C "$OTTIE_MAESTRO_PROJECT_PATH" commit -m "Initial commit" >/dev/null
else
  PROJECT_PARENT=""
fi

export OTTIE_MAESTRO_PROJECT_NAME="${OTTIE_MAESTRO_PROJECT_NAME:-$(basename "$OTTIE_MAESTRO_PROJECT_PATH")}"

echo "=== Workspace Create Android Crash Harness ==="
echo "Output dir: $OUT_DIR"
echo "App id: $OTTIE_MAESTRO_APP_ID"
echo "Android direct endpoint: $OTTIE_MAESTRO_DIRECT_ENDPOINT"
echo "Daemon websocket: $OTTIE_MAESTRO_DAEMON_WS_URL"
echo "Project: $OTTIE_MAESTRO_PROJECT_PATH"
echo "Project name: $OTTIE_MAESTRO_PROJECT_NAME"
...
const projectPath = process.env.OTTIE_MAESTRO_PROJECT_PATH;
const daemonUrl = process.env.OTTIE_MAESTRO_DAEMON_WS_URL;
