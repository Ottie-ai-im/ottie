...
import { AssistantsSection } from "@/screens/settings/assistants-section";
  ...
  return (
    <>
      <SettingsSection title="Operations">
        ...
        <InjectOttieToolsCard serverId={host.serverId} />
      </SettingsSection>
      {isLocalDaemon ? (
...
function RestartDaemonCard({ host }: { host: HostProfile }) {
  ...
  const handleRestart = useCallback(() => {
    ...
    if (!isHostConnected()) {
      Alert.alert(
        "Host offline",
        "This host is offline. Ottie reconnects automatically—wait until it's back online before restarting.",
      );
      return;
    }
      ...
      .then((confirmed) => {
        ...
        void daemonClient
          ...
          .catch((error) => {
            ...
            Alert.alert(
              "Error",
              "Failed to send the restart request. Ottie reconnects automatically—try again once the host shows as online.",
            );
          });
...
function InjectOttieToolsCard({ serverId }: { serverId: string }) {
  ...
  return (
    <View style={settingsStyles.card} testID="host-page-inject-mcp-card">
      <View style={settingsStyles.row}>
        <View style={settingsStyles.rowContent}>
          <Text style={settingsStyles.rowTitle}>Inject Ottie tools</Text>
          <Text style={settingsStyles.rowHint}>
            Automatically inject Ottie MCP tools into new agents
          </Text>
        </View>
        <SegmentedControl
          size="sm"
