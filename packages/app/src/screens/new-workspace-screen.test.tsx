...
import type { CreateOttieWorktreeInput } from "@server/client/daemon-client";
...
const {
  ...
  const hoistedCreatedWorkspace = {
    id: "workspace-1",
    workspaceDirectory: "/repo/.ottie/worktrees/workspace-1",
  };
  ...
  const hoistedMockClient = {
    ...
    createOttieWorktree: vi.fn(async (_input: CreateOttieWorktreeInput) => ({
...
beforeEach(() => {
  ...
  mockClient.createOttieWorktree.mockClear();
...
type CreateOttieWorktreeArg = Parameters<typeof mockClient.createOttieWorktree>[0];
...
function firstCreateWorktreeCall(): CreateOttieWorktreeArg {
  const calls = mockClient.createOttieWorktree.mock.calls;
  const firstCall = calls[0];
  if (!firstCall) throw new Error("createOttieWorktree not called");
  return firstCall[0];
}
...
describe("NewWorkspaceScreen picker payload", () => {
  ...
  it("sends no new fields when nothing is selected (default)", async () => {
    ...
    expect(mockClient.createOttieWorktree).toHaveBeenCalledTimes(1);
  ...
  it("preserves and locks the composer and picker while worktree creation is pending, then unlocks on error", async () => {
    ...
    mockClient.createOttieWorktree.mockImplementationOnce(async () => await createWorktree.promise);
    renderScreen();
    await flush();
