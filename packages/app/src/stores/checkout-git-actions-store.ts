...
function invalidateWorktreeList() {
  void appQueryClient.invalidateQueries({
    predicate: (query) =>
      Array.isArray(query.queryKey) && query.queryKey[0] === "ottieWorktreeList",
  });
  void appQueryClient.invalidateQueries({
    predicate: (query) =>
      Array.isArray(query.queryKey) && query.queryKey[0] === "sidebarOttieWorktreeList",
  });
}
  ...
  appQueryClient.setQueriesData(
    {
      predicate: (query) =>
        ...
        query.queryKey[0] === "ottieWorktreeList" &&
  ...
  appQueryClient.setQueriesData(
    {
      predicate: (query) =>
        Array.isArray(query.queryKey) &&
        query.queryKey[0] === "sidebarOttieWorktreeList" &&
        query.queryKey[1] === serverId,
    },
    removeFromList,
  );
...
function isWorktreeListQuery(input: { queryKey: QueryKey; serverId: string }): boolean {
  return (
    ...
    (input.queryKey[0] === "ottieWorktreeList" ||
      input.queryKey[0] === "sidebarOttieWorktreeList") &&
  ...
  archiveWorktree: async ({ serverId, cwd, worktreePath }) => {
    await runCheckoutAction({
      ...
      run: async () => {
        ...
        try {
          const payload = await client.archiveOttieWorktree({ worktreePath });
          if (payload.error) {
            throw new Error(payload.error.message);
          }
        } catch (error) {
