...
const originalGlobals: GlobalSnapshot = {
  ...
  ottieDesktop:
    typeof globalThis.window === "undefined"
      ? undefined
      : (globalThis.window as { ottieDesktop?: unknown }).ottieDesktop,
};

function ensureWindow(): { ottieDesktop?: unknown } {
  const existingWindow = (globalThis as { window?: { ottieDesktop?: unknown } }).window;
  if (existingWindow) {
    return existingWindow;
  }

  const nextWindow: { ottieDesktop?: unknown } = {};
...
function restoreGlobals(): void {
  ...
  if (typeof globalThis.window !== "undefined") {
    (globalThis.window as { ottieDesktop?: unknown }).ottieDesktop = originalGlobals.ottieDesktop;
  }
}
