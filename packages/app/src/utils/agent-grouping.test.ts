  ...
  it("shows owner/repo for GitHub remote keys", () => {
    expect(
      deriveProjectDisplayName({
        projectKey: "remote:github.com/ottie/ottie",
        projectName: "ottie",
      }),
    ).toBe("ottie/ottie");
  });
...
describe("groupAgents", () => {
  it("groups active agents by remote URL when available", () => {
    const agents = [
      makeAgent({ id: "a1", cwd: "/Users/me/dev/ottie" }),
      makeAgent({ id: "a2", cwd: "/Users/me/dev/ottie-fix/worktree" }),
    ];

    const { activeGroups } = groupAgents(agents, {
      getRemoteUrl: () => "git@github.com:ottie/ottie.git",
    });
