...
function inferMainRepoRootFromOttieWorktreePath(cwd: string): string | null {
  const normalizedPath = cwd.replace(/\\/g, "/");
  const marker = "/.ottie/worktrees";
...
export function resolveNewAgentWorkingDir(
  ...
  const explicitMainRepoRoot = checkout?.isOttieOwnedWorktree
  ...
  return inferMainRepoRootFromOttieWorktreePath(cwd) ?? cwd;
}
