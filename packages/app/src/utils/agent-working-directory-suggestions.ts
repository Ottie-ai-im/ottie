...
const OTTIE_WORKTREE_PATH_PATTERN = /(^|\/)\.ottie\/worktrees(\/|$)/;
...
export function collectAgentWorkingDirectorySuggestions(
  ...
  for (const source of sources) {
    ...
    if (isOttieOwnedWorktreePath(cwd)) {
      continue;
    }
...
function isOttieOwnedWorktreePath(cwd: string): boolean {
  return OTTIE_WORKTREE_PATH_PATTERN.test(cwd.replace(/\\/g, "/"));
}
