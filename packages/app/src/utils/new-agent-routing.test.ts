...
describe("buildNewAgentRoute", () => {
  ...
  it("encodes the working directory as a workspace path segment", () => {
    expect(buildNewAgentRoute("srv-1", "/Users/me/dev/ottie")).toBe(
...
describe("resolveNewAgentWorkingDir", () => {
  ...
  it("falls back to repo root when checkout metadata is unavailable", () => {
    expect(resolveNewAgentWorkingDir("/repo/.ottie/worktrees/feature", null)).toBe("/repo");
  });

  it("supports windows-style ottie worktree paths without checkout metadata", () => {
    expect(resolveNewAgentWorkingDir("C:\\Users\\me\\repo\\.ottie\\worktrees\\feature", null)).toBe(
      "C:\\Users\\me\\repo",
    );
  });

  it("returns the main repo root for ottie-owned worktrees", () => {
    const checkout = {
      isOttieOwnedWorktree: true,
      worktreeRoot: "/repo/.ottie/worktrees/feature",
      mainRepoRoot: "/repo/main",
    } as unknown as CheckoutStatusPayload;

    expect(resolveNewAgentWorkingDir("/repo/.ottie/worktrees/feature", checkout)).toBe(
      "/repo/main",
    );
  });
});
