...
import { forkOttieHomeMetadata, resolveOttieHomePath } from "./helpers/ottie-home-fork";
...
let ottieHome: string | null = null;
let fakeGhBinDir: string | null = null;
let relayProcess: ChildProcess | null = null;

function resolveOptionalOttieHomeEnv(value: string | undefined): string | null {
  const trimmed = value?.trim();
  if (!trimmed) {
    return null;
  }
  if (trimmed === "current") {
    return resolveOttieHomePath("~/.ottie");
  }
  return resolveOttieHomePath(trimmed);
}
...
async function createFakeGhBin(): Promise<string> {
  const binDir = await mkdtemp(path.join(tmpdir(), "ottie-e2e-gh-bin-"));
  ...
  console.log(JSON.stringify([
    {
      ...
      url: "https://github.com/ottie/ottie/pull/515",
      state: "OPEN",
...
function ensureRelayBuildArtifact(repoRoot: string): void {
  ...
  console.log("[e2e] Building @ottie/relay for daemon startup");
  execSync("npm run build --workspace=@ottie/relay", {
    cwd: repoRoot,
    stdio: "inherit",
  });
}
...
function loadPairingOfferFromCli(repoRoot: string, ottieHomePath: string): OfferPayload {
...
async function waitForPairingOfferFromCli(args: {
  ...
  while (Date.now() - start < timeoutMs) {
    try {
      return loadPairingOfferFromCli(args.repoRoot, args.ottieHome);
    } catch (error) {
      lastError = error;
      await sleep(100);
    }
  ...
  throw new Error(
    `Timed out waiting for \`ottie daemon pair --json\` to produce a pairing offer: ${
...
async function applyOttieHomeFork(targetHome: string): Promise<void> {
  const forkSourceHome = resolveOptionalOttieHomeEnv(process.env.E2E_FORK_OTTIE_HOME_FROM);
  if (!forkSourceHome) {
    return;
  }
  const forkResult = await forkOttieHomeMetadata({
    sourceHome: forkSourceHome,
    targetHome,
  });
  process.env.E2E_FORK_SOURCE_OTTIE_HOME = forkResult.sourceHome;
  process.env.E2E_FORK_TARGET_OTTIE_HOME = forkResult.targetHome;
  ...
  console.log(
    `[e2e] Forked Ottie metadata from ${forkResult.sourceHome} to ${forkResult.targetHome} ` +
  ...
  if (forkResult.skippedMissing.length > 0) {
    console.warn(
      `[e2e] Ottie metadata fork skipped missing paths: ${forkResult.skippedMissing.join(", ")}`,
    );
  }
}
...
async function resolveDictationConfig(): Promise<DictationConfig> {
  ...
  if (dictationProvider === "local" && !hasDefaultLocalModelsDir) {
    throw new Error(
      "OpenAI key is not usable and local speech models are unavailable at ~/.ottie/models/local-speech. " +
...
function startDaemon(args: DaemonSpawnArgs): ChildProcess {
  ...
  const child = spawn(tsxBin, ["src/server/index.ts"], {
    ...
    env: {
      ...
      OTTIE_HOME: args.ottieHome,
      OTTIE_SERVER_ID: "srv_e2e_test_daemon",
      OTTIE_LISTEN: `0.0.0.0:${args.port}`,
      OTTIE_RELAY_ENDPOINT: `127.0.0.1:${args.relayPort}`,
      OTTIE_CORS_ORIGINS: `http://localhost:${args.metroPort}`,
      OTTIE_DICTATION_ENABLED: openAiUsable ? "1" : "0",
      OTTIE_VOICE_MODE_ENABLED: openAiUsable ? "1" : "0",
      ...(openAiUsable
        ? {
            OTTIE_DICTATION_STT_PROVIDER: "openai",
            OTTIE_VOICE_STT_PROVIDER: "openai",
            OTTIE_VOICE_TTS_PROVIDER: "openai",
          }
        : {}),
      ...(localModelsDir ? { OTTIE_LOCAL_MODELS_DIR: localModelsDir } : {}),
...
async function performCleanup(shouldRemoveOttieHome: boolean): Promise<void> {
  await Promise.all([
  ...
  metroProcess = null;
  relayProcess = null;
  if (ottieHome && shouldRemoveOttieHome) {
    await rm(ottieHome, { recursive: true, force: true });
    ottieHome = null;
  } else if (ottieHome) {
    console.log(`[e2e] Preserving OTTIE_HOME: ${ottieHome}`);
  }
  if (fakeGhBinDir) {
...
export default async function globalSetup() {
  ...
  const requestedOttieHome = resolveOptionalOttieHomeEnv(process.env.E2E_OTTIE_HOME);
  const shouldRemoveOttieHome = !requestedOttieHome && process.env.E2E_KEEP_OTTIE_HOME !== "1";
  ottieHome = requestedOttieHome ?? (await mkdtemp(path.join(tmpdir(), "ottie-e2e-home-")));
  fakeGhBinDir = await createFakeGhBin();
  const metroLineBuffer = createLineBuffer();
  const daemonLineBuffer = createLineBuffer();

  await applyOttieHomeFork(ottieHome);

  const cleanup = () => performCleanup(shouldRemoveOttieHome);
  ...
  try {
    ...
    console.log(
      `[e2e] Test daemon started on port ${port}, Metro on port ${metroPort}, home: ${ottieHome}`,
    );
