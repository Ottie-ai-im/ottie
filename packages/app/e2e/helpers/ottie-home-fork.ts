...
export interface OttieHomeMetadataForkResult {
  sourceHome: string;
  targetHome: string;
  agentFiles: number;
  agentBytes: number;
...
export function resolveOttieHomePath(value: string): string {
  if (value === "~") {
    return homedir();
  }
  if (value.startsWith("~/")) {
...
export async function forkOttieHomeMetadata(input: {
  sourceHome: string;
  targetHome: string;
}): Promise<OttieHomeMetadataForkResult> {
  const sourceHome = resolveOttieHomePath(input.sourceHome);
  const targetHome = resolveOttieHomePath(input.targetHome);

  if (sourceHome === targetHome) {
    throw new Error("Refusing to fork Ottie metadata onto the same OTTIE_HOME.");
  }
