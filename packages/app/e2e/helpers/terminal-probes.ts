...
export async function installTerminalRenderProbe(page: Page): Promise<void> {
  await page.addInitScript(() => {
    interface ProbeTerm {
      ...
      __ottieRenderProbeWriteWrapped?: boolean;
    }
    ...
    const win = window as unknown as Record<string, unknown> & {
      ...
      __ottieTerminal?: ProbeTerm;
    };
    const existingDescriptor = Object.getOwnPropertyDescriptor(win, "__ottieTerminal");
    const getExisting = () =>
    ...
    Object.defineProperty(win, "__ottieTerminal", {
      configurable: true,
      get() {
        return probe.term;
      },
      set(next: ProbeTerm | undefined) {
        ...
        if (next?.write && !next.__ottieRenderProbeWriteWrapped) {
          ...
          next.__ottieRenderProbeWriteWrapped = true;
        }
      },
    });
  });
...
export async function installTerminalKeystrokeStressProbe(page: Page): Promise<void> {
  await page.addInitScript(() => {
    ...
    const existingDescriptor = Object.getOwnPropertyDescriptor(window, "__ottieTerminal");
    const getExisting = () =>
      existingDescriptor?.get ? existingDescriptor.get.call(window) : existingDescriptor?.value;

    let terminal = getExisting();
    Object.defineProperty(window, "__ottieTerminal", {
      configurable: true,
      get() {
        return terminal;
      },
      set(next: {
        write?: (data: string | Uint8Array, callback?: () => void) => void;
        __ottieKeystrokeProbeWriteWrapped?: boolean;
      }) {
        terminal = next;
        if (next?.write && !next.__ottieKeystrokeProbeWriteWrapped) {
          const originalWrite = next.write.bind(next);
          next.write = (data: string | Uint8Array, callback?: () => void) => {
            const text =
              typeof data === "string" ? data : new TextDecoder().decode(data as Uint8Array);
          ...
          next.__ottieKeystrokeProbeWriteWrapped = true;
        }
      },
    });
  });
