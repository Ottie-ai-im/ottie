...
export const createTempGitRepo = async (
  ...
  execSync('git config user.email "e2e@ottie.test"', { cwd: repoPath, stdio: "ignore" });
  execSync('git config user.name "Ottie E2E"', { cwd: repoPath, stdio: "ignore" });
  execSync("git config commit.gpgsign false", { cwd: repoPath, stdio: "ignore" });
  await writeFile(path.join(repoPath, "README.md"), "# Temp Repo\n");
  if (options?.ottieConfig) {
    await writeFile(
      path.join(repoPath, "ottie.json"),
      JSON.stringify(options.ottieConfig, null, 2),
    );
  }
  ...
  if (options?.ottieConfig) {
    execSync("git add ottie.json", { cwd: repoPath, stdio: "ignore" });
  }
  for (const file of options?.files ?? []) {
  ...
  for (const branch of branches) {
    ...
    const markerPath = `.ottie-e2e-${branch.replace(/[^a-zA-Z0-9._-]/g, "-")}.txt`;
