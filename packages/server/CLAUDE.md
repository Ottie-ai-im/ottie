# AGENTS.md — Ottie Server Development Guide

For AI coding agents working in `packages/server`. Supplements [CLAUDE.md](../CLAUDE.md) at the repo root.

## Project Overview

Ottie is a mobile + CLI app for monitoring and controlling local AI coding agents (Claude Code, Codex, OpenCode). The daemon runs on your machine, manages agent processes, and streams their output over WebSocket to clients.
...
```bash
npm run build --workspace=@ottie/relay    # Rebuild relay before daemon
npm run build --workspace=@ottie/server   # Rebuild server
...
Daemon logs: `$OTTIE_HOME/daemon.log`
...
```bash
tail -f $OTTIE_HOME/daemon.log      # Daemon logs
