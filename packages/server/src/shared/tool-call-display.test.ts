...
describe("shared tool-call display mapping", () => {
  ...
  it("builds display model for worktree setup detail", () => {
    const display = buildToolCallDisplayModel({
      name: "ottie_worktree_setup",
      status: "running",
      error: null,
      detail: {
        type: "worktree_setup",
        worktreePath: "/tmp/repo/.ottie/worktrees/repo/branch",
        ...
        commands: [
          {
            index: 1,
            command: "npm install",
            cwd: "/tmp/repo/.ottie/worktrees/repo/branch",
            log: "==> [1/1] Running: npm install\n",
            status: "running",
            exitCode: null,
          },
  ...
  it("humanizes Ottie MCP tool names (Claude Code format)", () => {
    const display = buildToolCallDisplayModel({
      name: "mcp__ottie__create_agent",
      status: "running",
      error: null,
  ...
  it("humanizes Ottie MCP tool names (Codex format)", () => {
    const display = buildToolCallDisplayModel({
      name: "ottie.create_agent",
      status: "running",
      error: null,
      detail: { type: "unknown", input: null, output: null },
    });
  ...
  it("humanizes list_agents Ottie tool", () => {
    const display = buildToolCallDisplayModel({
      name: "mcp__ottie__list_agents",
      status: "running",
      error: null,
      detail: { type: "unknown", input: null, output: null },
    });
