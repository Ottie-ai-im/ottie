...
import {
  ...
  readOttieWorktreeMetadata,
  readOttieWorktreeRuntimePort,
  writeOttieWorktreeMetadata,
  writeOttieWorktreeRuntimeMetadata,
} from "./worktree-metadata.js";
import { runGitCommand } from "./run-git-command.js";
import { resolveOttieHome } from "../server/ottie-home.js";
import { ensureNodePtySpawnHelperExecutableForCurrentPlatform } from "../terminal/terminal.js";
import { parseGitRevParsePath, resolveGitRevParsePath } from "./git-rev-parse-path.js";

interface OttieConfig {
  worktree?: {
    setup?: string[];
    teardown?: string[];
...
export interface WorktreeRuntimeEnv {
  [key: string]: string;
  OTTIE_SOURCE_CHECKOUT_PATH: string;
  OTTIE_ROOT_PATH: string;
  OTTIE_WORKTREE_PATH: string;
  OTTIE_BRANCH_NAME: string;
  OTTIE_WORKTREE_PORT: string;
}
...
export interface OttieWorktreeInfo {
  path: string;
  createdAt: string;
  branchName?: string;
  head?: string;
...
export interface OttieWorktreeOwnership {
  allowed: boolean;
  repoRoot?: string;
  worktreeRoot?: string;
  worktreePath?: string;
...
function readOttieConfig(repoRoot: string): OttieConfig | null {
  const ottieConfigPath = join(repoRoot, "ottie.json");
  if (!existsSync(ottieConfigPath)) {
    return null;
  }
  try {
    return JSON.parse(readFileSync(ottieConfigPath, "utf8"));
  } catch {
    throw new Error(`Failed to parse ottie.json`);
  }
}

export function getWorktreeSetupCommands(repoRoot: string): string[] {
  const config = readOttieConfig(repoRoot);
...
export async function runWorktreeSetupCommands(options: {
  ...
  // Read ottie.json from the worktree (it will have the same content as the source repo)
...
export async function resolveWorktreeRuntimeEnv(options: {
  ...
  let worktreePort = readOttieWorktreeRuntimePort(options.worktreePath);
  if (worktreePort === null) {
    worktreePort = await getAvailablePort();
    const metadata = readOttieWorktreeMetadata(options.worktreePath);
    if (metadata) {
      writeOttieWorktreeRuntimeMetadata(options.worktreePath, { worktreePort });
    }
  } else {
  ...
  return {
    ...
    // source checkout.
    OTTIE_SOURCE_CHECKOUT_PATH: repoRootPath,
    // Backward-compatible alias.
    OTTIE_ROOT_PATH: repoRootPath,
    OTTIE_WORKTREE_PATH: options.worktreePath,
    OTTIE_BRANCH_NAME: branchName,
    OTTIE_WORKTREE_PORT: String(worktreePort),
  };
}
...
export async function runWorktreeTeardownCommands(options: {
  ...
  const branchName =
    options.branchName ?? (await resolveBranchNameForWorktreePath(options.worktreePath));
  const worktreePort = readOttieWorktreeRuntimePort(options.worktreePath);
  ...
  const teardownEnv: NodeJS.ProcessEnv = {
    ...
    OTTIE_WORKTREE_PATH: options.worktreePath,
    OTTIE_BRANCH_NAME: branchName,
    ...(worktreePort !== null ? { OTTIE_WORKTREE_PORT: String(worktreePort) } : {}),
  };
...
export async function getOttieWorktreesRoot(cwd: string, ottieHome?: string): Promise<string> {
  const home = ottieHome ? resolve(ottieHome) : resolveOttieHome();
...
export async function computeWorktreePath(
  ...
  const worktreesRoot = await getOttieWorktreesRoot(cwd, ottieHome);
...
export async function isOttieOwnedWorktreeCwd(
  cwd: string,
  options?: { ottieHome?: string },
): Promise<OttieWorktreeOwnership> {
  ...
  const ottieHome = options?.ottieHome ? resolve(options.ottieHome) : resolveOttieHome();
  const ottieWorktreesPrefix = normalizePathForOwnership(join(ottieHome, "worktrees")) + sep;

  // Ownership is defined by the path living under $OTTIE_HOME/worktrees/<hash>/<slug>[/...].
  // The <hash>/<slug> prefix is Ottie-private — nothing else writes there — so the
  // path shape alone is sufficient proof of ownership, even when git has already
  // forgotten about the worktree.
  if (!resolvedCwd.startsWith(ottieWorktreesPrefix)) {
    return {
      allowed: false,
  ...
  const relative = resolvedCwd.slice(ottieWorktreesPrefix.length);
  ...
  const worktreesRoot = join(ottieHome, "worktrees", parts[0]!);
  return {
    allowed: true,
    ...(repoRoot !== undefined ? { repoRoot } : {}),
...
type ParsedOttieWorktreeInfo = Omit<OttieWorktreeInfo, "createdAt">;

function parseWorktreeList(output: string): ParsedOttieWorktreeInfo[] {
  const entries: ParsedOttieWorktreeInfo[] = [];
  let current: ParsedOttieWorktreeInfo | null = null;
...
export async function listOttieWorktrees({
  cwd,
  ottieHome,
}: {
  cwd: string;
  ottieHome?: string;
}): Promise<OttieWorktreeInfo[]> {
  const worktreesRoot = await getOttieWorktreesRoot(cwd, ottieHome);
...
export async function resolveExistingWorktreeForSlug({
  ...
  const worktrees = await listOttieWorktrees({
    cwd: repoRoot,
    ottieHome,
  });
...
export async function resolveOttieWorktreeRootForCwd(
  cwd: string,
  options?: { ottieHome?: string },
  ...
  const worktreesRoot = await getOttieWorktreesRoot(cwd, options?.ottieHome);
  ...
  const knownWorktrees = await listOttieWorktrees({
    cwd,
...
export async function deleteOttieWorktree({
  cwd,
  worktreePath,
  worktreeSlug,
  worktreesRoot,
  ...
  if (worktreesRoot) {
    ...
    resolvedWorktreesRoot = await getOttieWorktreesRoot(cwd, ottieHome);
  } else {
    throw new Error("cwd or worktreesRoot is required to delete a Ottie worktree");
  }
  ...
  const resolvedWorktree =
    (await resolveOttieWorktreeRootForCwd(requestedPath, { ottieHome }))?.worktreePath ??
    resolvedRequested;

  if (!resolvedWorktree.startsWith(resolvedRoot)) {
    throw new Error("Refusing to delete non-Ottie worktree");
  }
...
export const createWorktree = async ({
  ...
  let worktreePath = join(await getOttieWorktreesRoot(cwd, ottieHome), worktreeSlug);
  ...
  writeOttieWorktreeMetadata(worktreePath, { baseRefName: sourcePlan.metadataBaseRefName });
    ...
    case "checkout-github-pr": {
      ...
      return {
        ...
        ...(source.pushRemoteUrl
          ? {
              pushRemote: {
                name: `ottie-pr-${source.githubPrNumber}`,
...
function normalizeRequiredBaseBranch(baseBranch: string): string {
  ...
  if (!normalizedBaseBranch) {
    throw new Error("Base branch is required when creating a Ottie worktree");
  }
  if (normalizedBaseBranch === "HEAD") {
    throw new Error("Base branch cannot be HEAD when creating a Ottie worktree");
  }
