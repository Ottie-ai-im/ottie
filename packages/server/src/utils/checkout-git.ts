...
import { isOttieOwnedWorktreeCwd } from "./worktree.js";
import { requireOttieWorktreeBaseRefName } from "./worktree-metadata.js";
...
export interface CheckoutStatusGitNonOttie {
  isGit: true;
  repoRoot: string;
...
export interface CheckoutStatusGitOttie {
  isGit: true;
  repoRoot: string;
  mainRepoRoot: string;
  currentBranch: string | null;
...
export type CheckoutStatusGit = CheckoutStatusGitNonOttie | CheckoutStatusGitOttie;
...
export async function getMainRepoRoot(cwd: string): Promise<string> {
  ...
  const nonBareNonOttie = worktrees.filter((wt) => !wt.isBare && !isOttieWorktreePath(wt.path));
  const childrenOfBareRepo = nonBareNonOttie.filter((wt) => isDescendantPath(wt.path, normalized));
  const mainChild = childrenOfBareRepo.find((wt) => basename(wt.path) === "main");
  return mainChild?.path ?? childrenOfBareRepo[0]?.path ?? nonBareNonOttie[0]?.path ?? normalized;
}
...
/** Check whether a path contains a `.ottie/worktrees/` segment (both `/` and `\`). */
export function isOttieWorktreePath(p: string): boolean {
  return /[/\\]\.ottie[/\\]worktrees[/\\]/.test(p);
}
...
type ConfiguredBaseRefForCwd =
  | { baseRef: null; isOttieOwnedWorktree: false }
  | { baseRef: string; isOttieOwnedWorktree: true };
...
async function getConfiguredBaseRefForCwd(
  ...
  if (!/[\\/]worktrees[\\/]/.test(cwd)) {
    return { baseRef: null, isOttieOwnedWorktree: false };
  }

  const ownership = await isOttieOwnedWorktreeCwd(cwd, { ottieHome: context?.ottieHome });
  ...
  return {
    baseRef: requireOttieWorktreeBaseRefName(worktreeRoot),
    isOttieOwnedWorktree: true,
  };
}
...
async function resolvePullRequestStatusLookupTarget(
  ...
  if (!remoteName?.startsWith("ottie-pr-")) {
...
export async function getCheckoutStatus(
  ...
  if (configured.isOttieOwnedWorktree) {
...
async function resolveCheckoutDiffRefs(
  ...
  if (configured.isOttieOwnedWorktree && compare.baseRef && compare.baseRef !== baseRef) {
...
export async function mergeToBase(
  ...
  if (configured.isOttieOwnedWorktree && options.baseRef && options.baseRef !== baseRef) {
...
export async function createPullRequest(
  ...
  if (configured.isOttieOwnedWorktree && options.base && options.base !== base) {
