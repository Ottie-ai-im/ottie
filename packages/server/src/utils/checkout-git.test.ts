...
function createGitHubServiceForStatus(
  ...
  return {
    ...
    getPullRequest: async () => ({
      number: 1,
      title: "PR",
      url: "https://github.com/ottie/ottie/pull/1",
      state: "OPEN",
      body: null,
      baseRefName: "main",
      headRefName: "feature",
...
function createPullRequestStatus(overrides?: Partial<GitHubCurrentPullRequestStatus>) {
  return {
    url: "https://github.com/ottie/ottie/pull/123",
    title: "Ship feature",
    state: "open",
    baseRefName: "main",
    headRefName: "feature",
...
`;
  ...
  it("returns checkout root metadata for normal repos", async () => {
    ...
    expect(status.isOttieOwnedWorktree).toBe(false);
  ...
  it("handles status/diff/commit in a .ottie worktree", async () => {
    ...
    const status = await getCheckoutStatus(result.worktreePath, { ottieHome });
    expect(status.isGit).toBe(true);
    expect(status.repoRoot).toBe(result.worktreePath);
    expect(status.isDirty).toBe(true);
    expect(status.isOttieOwnedWorktree).toBe(true);
    expect(status.mainRepoRoot).toBe(repoDir);

    const diff = await getCheckoutDiff(result.worktreePath, { mode: "uncommitted" }, { ottieHome });
    expect(diff.diff).toContain("-hello");
    expect(diff.diff).toContain("+worktree change");

    await commitAll(result.worktreePath, "worktree update");

    const cleanStatus = await getCheckoutStatus(result.worktreePath, { ottieHome });
    expect(cleanStatus.isDirty).toBe(false);
  ...
  it("returns checkout root metadata for .ottie worktrees", async () => {
    const result = await createLegacyWorktreeForTest({
      branchName: "main",
      cwd: repoDir,
      baseBranch: "main",
  ...
  it("returns mainRepoRoot pointing to first non-bare worktree for bare repos", async () => {
    ...
    const status = await getCheckoutStatus(worktree.worktreePath, { ottieHome });
    expect(status.isGit).toBe(true);
    expect(status.isOttieOwnedWorktree).toBe(true);
  ...
  it("merges the current branch into base from a worktree checkout", async () => {
    ...
    await mergeToBase(worktree.worktreePath, { baseRef: "main" }, { ottieHome });
    ...
    const statusAfterMerge = await getCheckoutStatus(worktree.worktreePath, { ottieHome });
  ...
  it("reports the base worktree cwd when merge-to-base mutates a separate checkout", async () => {
    ...
    const mutatedCwd = await mergeToBase(featureWorktree.worktreePath, {}, { ottieHome });
  ...
  it("disables GitHub features when gh is unavailable", async () => {
    execSync("git remote add origin https://github.com/ottie/ottie.git", { cwd: repoDir });
          ...
          checks: [
            {
              name: "server-tests",
              status: "success",
              url: "https://github.com/ottie/ottie/actions/runs/123",
              workflow: "Server CI",
              duration: "2m 14s",
            },
          ],
  ...
  it("uses the tracked fork branch for PR worktree status lookup", async () => {
    ...
    execSync("git remote add origin https://github.com/ottie/ottie.git", { cwd: repoDir });
    execSync("git remote add ottie-pr-345 git@github.com:chethanuk/ottie.git", { cwd: repoDir });
    execSync("git config branch.chethanuk/main.remote ottie-pr-345", { cwd: repoDir });
    ...
    const github = createGitHubServiceForStatus(
      createPullRequestStatus({
        number: 345,
        url: "https://github.com/ottie/ottie/pull/345",
        headRefName: "main",
      }),
      {
        onStatus: () => {},
  ...
  it("returns closed-unmerged PR status without marking it as merged", async () => {
    ...
    const status = await getPullRequestStatus(
      ...
      createGitHubServiceForStatus(
        createPullRequestStatus({
          url: "https://github.com/ottie/ottie/pull/999",
  ...
  it("expires cached PR status after the TTL", async () => {
    ...
    try {
      ...
      github.getCurrentPullRequestStatus = async () => {
        callCount += 1;
        return createPullRequestStatus({
          url: `https://github.com/ottie/ottie/pull/${callCount}`,
        });
      };
      const first = await getPullRequestStatus(repoDir, github);
      await sleep(80);
  ...
  it("uses stored baseRefName for Ottie worktrees (no heuristics)", async () => {
    ...
    const baseDiff = await getCheckoutDiff(worktree.worktreePath, { mode: "base" }, { ottieHome });
    expect(baseDiff.diff).toContain("feature.txt");
    expect(baseDiff.diff).not.toContain("file.txt");
  });

  it("excludes dirty working tree changes from Ottie worktree base diffs", async () => {
    const worktree = await createLegacyWorktreeForTest({
      branchName: "feature",
      cwd: repoDir,
      baseBranch: "main",
  ...
  it("merges to stored baseRefName when baseRef is not provided", async () => {
    ...
    // Create a Ottie worktree configured to use develop as base.
    const worktree = await createLegacyWorktreeForTest({
      branchName: "feature",
      cwd: repoDir,
      baseBranch: "develop",
    ...
    await mergeToBase(worktree.worktreePath, {}, { ottieHome });
  ...
  it("throws if Ottie worktree base metadata is missing", async () => {
    const worktree = await createLegacyWorktreeForTest({
      branchName: "main",
      cwd: repoDir,
      baseBranch: "main",
    ...
    const metadataPath = getOttieWorktreeMetadataPath(worktree.worktreePath);
    rmSync(metadataPath, { force: true });

    await expect(getCheckoutStatus(worktree.worktreePath, { ottieHome })).rejects.toThrow(/base/i);
    await expect(
      getCheckoutDiff(worktree.worktreePath, { mode: "base" }, { ottieHome }),
    ).rejects.toThrow(/base/i);
    await expect(mergeToBase(worktree.worktreePath, {}, { ottieHome })).rejects.toThrow(/base/i);
  });
  ...
  describe("parseWorktreeList", () => {
    it("parses porcelain worktree output", () => {
      const output = [
        ...
        "",
        "worktree /home/user/.ottie/worktrees/feature",
      ...
      expect(entries[1]).toEqual({
        path: "/home/user/.ottie/worktrees/feature",
  ...
  describe("isOttieWorktreePath", () => {
    it("matches Unix .ottie/worktrees/ paths", () => {
      expect(isOttieWorktreePath("/home/user/.ottie/worktrees/feature")).toBe(true);
    });

    it("matches Windows .ottie\\worktrees\\ paths", () => {
      expect(isOttieWorktreePath("C:\\Users\\dev\\.ottie\\worktrees\\feature")).toBe(true);
    });

    it("rejects paths without .ottie/worktrees segment", () => {
      expect(isOttieWorktreePath("/home/user/repo")).toBe(false);
      expect(isOttieWorktreePath("C:\\Users\\dev\\repo")).toBe(false);
    });
  });
