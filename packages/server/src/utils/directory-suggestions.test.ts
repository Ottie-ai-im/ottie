...
describe("searchHomeDirectories", () => {
  ...
  beforeEach(() => {
    ...
    mkdirSync(path.join(homeDir, "projects", "ottie"), { recursive: true });
  ...
  it("returns only existing directories", async () => {
    ...
    expect(results).toContain(path.join(homeDir, "projects", "ottie"));
  ...
  it("supports home-relative path query syntax", async () => {
    ...
    expect(results).toEqual([path.join(homeDir, "projects", "ottie")]);
  });
...
describe("searchWorkspaceEntries", () => {
  ...
  beforeEach(() => {
    ...
    writeFileSync(path.join(workspaceDir, "README.md"), "# ottie\n");
    writeFileSync(
