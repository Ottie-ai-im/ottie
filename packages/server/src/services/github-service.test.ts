    ...
    data: {
      repository: {
        pullRequest: {
          ...
          headRepository: {
            sshUrl: "git@github.com:therainisme/ottie.git",
            url: "https://github.com/therainisme/ottie",
          },
        },
      },
    },
...
describe("GitHubService", () => {
  ...
  it("loads pull request checkout target details through GraphQL", async () => {
    ...
    await expect(
      ...
      headRepositorySshUrl: "git@github.com:therainisme/ottie.git",
      headRepositoryUrl: "https://github.com/therainisme/ottie",
      isCrossRepository: true,
    });
    ...
    expect(runner.calls[1]?.args).toContain("owner=ottie");
    expect(runner.calls[1]?.args).toContain("name=ottie");
  ...
  it("resolves GitHub repos from a WorkspaceGitService-owned remote URL", async () => {
    const workspaceGitService = {
      resolveRepoRemoteUrl: async (cwd: string) => {
        ...
        return "git@github.com:ottie/ottie.git";
      },
    };
