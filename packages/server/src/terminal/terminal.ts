...
function resolveZshShellIntegrationRuntimeDir(): string {
  ...
  return join(tmpdir(), `${username}-ottie-zsh`);
}
...
function prepareZshShellIntegrationRuntimeDir(sourceDir = resolveZshShellIntegrationDir()): string {
  ...
  copyFileSync(
    join(readableSourceDir, "ottie-integration.zsh"),
    join(runtimeDir, "ottie-integration.zsh"),
  );
  return runtimeDir;
}
...
export function buildTerminalEnvironment(
  ...
  return {
    ...baseEnv,
    OTTIE_ZSH_ZDOTDIR: originalZdotdir,
