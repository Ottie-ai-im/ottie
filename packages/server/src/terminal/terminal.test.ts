...
describe("createTerminal", () => {
  ...
  it("sets zsh wrapper env when spawning zsh", () => {
    const resolvedEnv = buildTerminalEnvironment({
      ...
      env: {
        HOME: "/tmp/ottie-home",
        ZDOTDIR: "/tmp/ottie-zdotdir",
      },
    });

    expect(resolvedEnv.TERM).toBe("xterm-256color");
    expect(resolvedEnv.OTTIE_ZSH_ZDOTDIR).toBe("/tmp/ottie-zdotdir");
    expect(resolvedEnv.ZDOTDIR).not.toBe("/tmp/ottie-zdotdir");
    expect(existsSync(join(resolvedEnv.ZDOTDIR, ".zshenv"))).toBe(true);
    expect(existsSync(join(resolvedEnv.ZDOTDIR, "ottie-integration.zsh"))).toBe(true);
  });
...
describe("terminal title", () => {
  it.skipIf(!hasZsh)("restores the user's ZDOTDIR through the zsh wrapper", async () => {
    ...
    writeFileSync(join(realZdotdir, ".zshenv"), "export OTTIE_TEST_REAL_ZDOTDIR=1\n");

    const session = trackSession(
      await createTerminal({
        cwd: homeDir,
        command: "/bin/zsh",
        args: ["-c", 'printf \'%s\\n%s\\n\' "${ZDOTDIR-}" "${OTTIE_TEST_REAL_ZDOTDIR-}"'],
        env: {
          HOME: homeDir,
          ZDOTDIR: realZdotdir,
        },
  ...
  it.skipIf(!hasZsh)("loads the user's zsh prompt when the integration dir is packaged", () => {
    ...
    temporaryDirs.push(homeDir);
    writeFileSync(join(homeDir, ".zshrc"), "PS1='OTTIE_CUSTOM_PROMPT> '\n");

    const fakeAppRoot = join(homeDir, "Ottie.app", "Contents", "Resources");
