  ...
  mock.triggerMessage(
    wrapSessionMessage({
      ...
      payload: {
        ...
        detail: {
          type: "worktree_setup",
          worktreePath: "/tmp/project/.ottie/worktrees/feature-a",
          ...
          commands: [
            {
              index: 1,
              command: "npm install",
              cwd: "/tmp/project/.ottie/worktrees/feature-a",
              log: "phase-one\n",
              status: "running",
              exitCode: null,
            },
...
test("sends structured attachments with create_ottie_worktree_request", async () => {
  const logger = createMockLogger();
  const mock = createMockTransport();
  ...
  const createPromise = client.createOttieWorktree({
    cwd: "/tmp/project",
    worktreeSlug: "review-pr-123",
    attachments: [
      {
...
test("sends worktree base-ref fields in create_ottie_worktree_request", async () => {
  const logger = createMockLogger();
  const mock = createMockTransport();
...
test("omitting create_ottie_worktree_request worktree base-ref fields preserves legacy wire shape", async () => {
  const logger = createMockLogger();
  const mock = createMockTransport();
  ...
  mock.triggerMessage(
    JSON.stringify({
      ...
      message: {
        ...
        payload: {
          directories: ["/Users/test/projects/ottie"],
