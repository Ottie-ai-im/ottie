...
import type {
  ...
  OttieWorktreeListResponse,
  OttieWorktreeArchiveResponse,
  ProjectIconResponse,
...
export interface CreateOttieWorktreeInput extends Pick<
  CreateOttieWorktreeRequest,
...
type OttieWorktreeListPayload = OttieWorktreeListResponse["payload"];
type OttieWorktreeArchivePayload = OttieWorktreeArchiveResponse["payload"];
type CreateOttieWorktreePayload = Extract<
  SessionOutboundMessage,
...
export class DaemonClient {
  ...
  async friendPairGenerate(params?: {
    requestId?: string;
    timeoutMs?: number;
    ttlMs?: number;
  }): Promise<{
    offer: import("../server/identity/friend-pair-types.js").FriendPairOffer | null;
    deepLink: string | null;
    error: string | null;
  }> {
    const requestId = this.createRequestId(params?.requestId);
    return this.sendRequest({
      ...
      timeout: params?.timeoutMs ?? 5000,
      select: (msg) => {
        if (msg.type !== "friend/pair/generate/response") return null;
        if (msg.payload.requestId !== requestId) return null;
        return {
          offer: msg.payload.offer,
          deepLink: msg.payload.deepLink,
  ...
  async friendPairCancel(
    ...
    return this.sendRequest({
      requestId,
      message: { type: "friend/pair/cancel", requestId, nonceB64 },
      timeout: params?.timeoutMs ?? 5000,
      select: (msg) => {
        if (msg.type !== "friend/pair/cancel/response") return null;
        if (msg.payload.requestId !== requestId) return null;
        return { cancelled: msg.payload.cancelled, error: msg.payload.error };
      },
    });
  ...
  async getOttieWorktreeList(
    input: { cwd?: string; repoRoot?: string },
    requestId?: string,
  ): Promise<OttieWorktreeListPayload> {
    return this.sendCorrelatedSessionRequest({
      requestId,
      message: {
      ...
      responseType: "ottie_worktree_list_response",
      timeout: 60000,
    });
  }

  async archiveOttieWorktree(
    input: { worktreePath?: string; repoRoot?: string; branchName?: string },
    requestId?: string,
  ): Promise<OttieWorktreeArchivePayload> {
    return this.sendCorrelatedSessionRequest({
      requestId,
      message: {
      ...
      responseType: "ottie_worktree_archive_response",
      timeout: 20000,
    });
  }

  async createOttieWorktree(
    input: CreateOttieWorktreeInput,
    requestId?: string,
  ): Promise<CreateOttieWorktreePayload> {
    return this.sendCorrelatedSessionRequest({
      requestId,
      message: {
      ...
      responseType: "create_ottie_worktree_response",
      timeout: 60000,
    });
  }
