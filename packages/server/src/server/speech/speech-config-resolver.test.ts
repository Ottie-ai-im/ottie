...
describe("resolveSpeechConfig", () => {
  test("resolves local-first defaults without env overrides", () => {
    const ottieHome = "/tmp/ottie-home";
    ...
    expect(result.speech.local).toEqual({
      modelsDir: path.join(ottieHome, "models", "local-speech"),
      models: {
  ...
  test("resolves feature-scoped local model env vars", () => {
    ...
    const env = {
      OTTIE_DICTATION_LOCAL_STT_MODEL: "zipformer",
      OTTIE_VOICE_LOCAL_STT_MODEL: "parakeet",
      OTTIE_VOICE_LOCAL_TTS_MODEL: "kitten",
      OTTIE_VOICE_LOCAL_TTS_SPEAKER_ID: "5",
      OTTIE_VOICE_LOCAL_TTS_SPEED: "1.35",
      OTTIE_LOCAL_MODELS_DIR: "/tmp/models",
      OPENAI_API_KEY: "env-key",
      OTTIE_VOICE_STT_PROVIDER: "openai",
      OTTIE_DICTATION_STT_PROVIDER: "local",
      OTTIE_VOICE_TTS_PROVIDER: "local",
  ...
  test("ignores deprecated shared local model env vars", () => {
    const persisted = PersistedConfigSchema.parse({});
    const env = {
      OTTIE_LOCAL_STT_MODEL: "zipformer-bilingual-zh-en-2023-02-20",
      OTTIE_LOCAL_TTS_MODEL: "kitten-nano-en-v0_1-fp16",
    } as NodeJS.ProcessEnv;
