...
interface SessionTestAccess {
  ...
  handleCreateOttieWorktreeRequest(params: unknown): Promise<unknown>;
...
test("workspace reconciliation reports archived workspaces to subscribed clients", async () => {
  const missingCwd = path.join(tmpdir(), `ottie-missing-workspace-${Date.now()}`);
...
test("create ottie worktree request returns a registered workspace descriptor", async () => {
  const emitted: Array<{ type: string; payload: unknown }> = [];
  const tempDir = realpathSync(mkdtempSync(path.join(tmpdir(), "session-worktree-test-")));
  const repoDir = path.join(tempDir, "repo");
  const ottieHome = path.join(tempDir, "ottie-home");
  ...
  try {
    await session.handleCreateOttieWorktreeRequest({
  ...
  const response = emitted.find((message) => message.type === "create_ottie_worktree_response") as
    | {
...
test.skip("opening a new worktree reconciles older local workspaces into the remote project", async () => {
  ...
  const worktreeWorkspaceId = path.join(mainWorkspaceId, ".ottie", "worktrees", "feature-a");
  ...
  session.buildProjectPlacement = async (cwd: string) => ({
    ...
    checkout: {
      ...
      worktreeRoot: cwd,
      isOttieOwnedWorktree: cwd !== mainWorkspaceId,
