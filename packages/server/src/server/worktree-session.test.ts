...
import { archiveOttieWorktree } from "./ottie-worktree-archive-service.js";
import {
  buildAgentSessionConfig,
  handleOttieWorktreeArchiveRequest,
  handleOttieWorktreeListRequest,
  resolveGitCreateBaseBranch,
  runWorktreeSetupInBackground,
  handleCreateOttieWorktreeRequest,
...
import {
  createOttieWorktree as createOttieWorktreeService,
  type CreateOttieWorktreeFn,
} from "./ottie-worktree-service.js";
...
function createOttieWorktreeForTest(options: {
  ottieHome: string;
  events?: string[];
}): CreateOttieWorktreeFn {
  ...
  return (input, serviceOptions) => {
    ...
    return createOttieWorktreeService(input, {
      ...coreDeps,
...
describe("handleOttieWorktreeListRequest", () => {
  test("lists worktrees through the workspace git service", async () => {
    const emitted: SessionOutboundMessage[] = [];
    const workspaceGitService = {
      listWorktrees: vi.fn().mockResolvedValue([
        {
          path: "/tmp/ottie-home/worktrees/repo/feature",
    ...
    await handleOttieWorktreeListRequest(
      {
        emit: (message) => emitted.push(message),
        ottieHome: "/tmp/ottie-home",
        workspaceGitService: workspaceGitService as unknown as WorkspaceGitService,
      },
      {
        type: "ottie_worktree_list_request",
        cwd: "/tmp/repo",
    ...
    expect(emitted).toContainEqual({
      type: "ottie_worktree_list_response",
      payload: {
        worktrees: [
          {
            worktreePath: "/tmp/ottie-home/worktrees/repo/feature",
            createdAt: "2026-04-12T00:00:00.000Z",
            branchName: "feature",
            head: "abc123",
          },
...
function createGitRepo(options?: { ottieConfig?: Record<string, unknown> }) {
  ...
  if (options?.ottieConfig) {
    writeFileSync(path.join(repoDir, "ottie.json"), JSON.stringify(options.ottieConfig, null, 2));
  }
...
describe("runWorktreeSetupInBackground", () => {
  ...
  test("emits running then completed snapshots for no-setup workspaces without auto-starting scripts", async () => {
    ...
    const ottieHome = path.join(tempDir, ".ottie");
  ...
  test("archives the pending workspace and emits a failed snapshot when setup cannot start", async () => {
    ...
    writeFileSync(path.join(repoDir, "ottie.json"), "{ invalid json\n");
    execSync("git add ottie.json", { cwd: repoDir, stdio: "pipe" });
    ...
    expect(progressMessages[1]?.payload.error).toContain("Failed to parse ottie.json");
    expect(progressMessages[1]?.payload.detail.commands).toEqual([]);
    expect(snapshots.get(workspaceId)).toMatchObject({
      status: "failed",
      error: expect.stringContaining("Failed to parse ottie.json"),
    });
    ...
    const snapshots = new Map([
      [
        "ws-feature-a",
        {
          ...
          detail: {
            type: "worktree_setup",
            worktreePath: "/repo/.ottie/worktrees/feature-a",
...
describe("handleCreateOttieWorktreeRequest", () => {
  const cleanupPaths: string[] = [];
  ...
  test("checks out the GitHub PR branch when a github_pr attachment is present", async () => {
    ...
    const response = emitted.find(
      (
        message,
      ): message is Extract<SessionOutboundMessage, { type: "create_ottie_worktree_response" }> =>
        message.type === "create_ottie_worktree_response",
    );
  ...
  test("buildAgentSessionConfig checks out the GitHub PR branch for agent worktrees", async () => {
    ...
    const result = await buildAgentSessionConfig(
      {
        ottieHome: path.join(tempDir, ".ottie"),
        sessionLogger: createLogger(),
        createOttieWorktree: createOttieWorktreeForTest({
          ottieHome: path.join(tempDir, ".ottie"),
          events,
        }),
  ...
  test("buildAgentSessionConfig invalidates GitHub cache after branch setup mutations", async () => {
    ...
    const createOttieWorktree = vi.fn(async () => {
  ...
  test("createOttieWorktreeForTest forwards the default branch resolver for branch-off intents", async () => {
    const { tempDir, repoDir } = createGitRepo();
    cleanupPaths.push(tempDir);
    const ottieHome = path.join(tempDir, ".ottie");
    const resolveDefaultBranch = vi.fn(async () => "main");

    const result = await createOttieWorktreeForTest({ ottieHome })(
      {
        cwd: repoDir,
      ...
      await handleCreateOttieWorktreeRequest(
        {
          ...
          sessionLogger: createLogger(),
          emit: (message) => emitted.push(message),
          createOttieWorktree: createOttieWorktreeForTest({ ottieHome, events }),
  ...
  test("creates the worktree before emitting the response", async () => {
    ...
    try {
      await handleCreateOttieWorktreeRequest(
        {
          ...
          sessionLogger: createLogger(),
          emit: (message) => emitted.push(message),
          createOttieWorktree: async (input) => {
            const result = await createOttieWorktreeForTest({ ottieHome })(input);
      ...
      expect(emitted.map((message) => message.type).slice(0, 2)).toEqual([
        "create_ottie_worktree_response",
        "workspace_update",
      ]);
...
describe("archiveOttieWorktree", () => {
  ...
  test("runs agent close and terminal teardown concurrently and removes the worktree", async () => {
    ...
    const emitted: SessionOutboundMessage[] = [];
    const removedAgents = await archiveOttieWorktree(
      {
        ottieHome,
        github: createGitHubServiceStub(),
        agentManager: {
  ...
  test("succeeds when git has forgotten about the worktree (no repoRoot)", async () => {
    ...
    const emitted: SessionOutboundMessage[] = [];
    await handleOttieWorktreeArchiveRequest(
      {
        ottieHome,
        github: createGitHubServiceStub(),
        agentManager: {
      ...
      {
        type: "ottie_worktree_archive_request",
    ...
    const response = emitted.find(
      (
        message,
      ): message is Extract<SessionOutboundMessage, { type: "ottie_worktree_archive_response" }> =>
        message.type === "ottie_worktree_archive_response",
    );
