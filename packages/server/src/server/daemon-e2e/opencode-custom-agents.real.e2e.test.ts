...
async function createHarness(): Promise<{
  client: DaemonClient;
  daemon: Awaited<ReturnType<typeof createTestOttieDaemon>>;
}> {
  ...
  test("custom agents defined in opencode.json appear in availableModes", async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        agent: {
          "ottie-e2e-custom": {
            description: "Custom agent for Ottie daemon E2E test",
            mode: "primary",
          },
        },
      }),
    ...
    try {
      ...
      const custom = snapshot.availableModes.find((m) => m.id === "ottie-e2e-custom");
      expect(custom).toBeDefined();
      expect(custom!.description).toBe("Custom agent for Ottie daemon E2E test");
