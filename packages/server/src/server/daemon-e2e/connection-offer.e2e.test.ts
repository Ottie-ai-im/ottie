...
describe("ConnectionOfferV2 (daemon E2E)", () => {
  ...
  test("emits relay-only offer URL with stable serverId", async () => {
    process.env.OTTIE_PRIMARY_LAN_IP = "192.168.1.12";

    const { logger } = createCapturingLogger();

    const daemon = await createTestOttieDaemon({
      listen: "0.0.0.0",
      logger,
      relayEnabled: true,
    });
    ...
    try {
      ...
      expect(offerUrl.startsWith("https://app.ottie.app/#offer=")).toBe(true);
      ...
      expect(offer.relay.endpoint).toBe("relay.ottie.app:443");
  ...
  test("persists serverId and daemon keypair across daemon restarts", async () => {
    ...
    const tempHomeRoot = await mkdtemp(path.join(os.tmpdir(), "ottie-offer-home-"));

    const { logger: logger1 } = createCapturingLogger();
    const daemon1 = await createTestOttieDaemon({
      listen: "0.0.0.0",
      logger: logger1,
      relayEnabled: true,
      ottieHomeRoot: tempHomeRoot,
      cleanup: false,
    });
    ...
    try {
      ...
      const daemon2 = await createTestOttieDaemon({
        listen: "0.0.0.0",
        logger: logger2,
        relayEnabled: true,
        ottieHomeRoot: tempHomeRoot,
  ...
  test("respects --no-relay (CLI) by not emitting a pairing offer", async () => {
    ...
    const tempHome = await mkdtemp(path.join(os.tmpdir(), "ottie-offer-e2e-"));
    ...
    const env = {
      ...process.env,
      OTTIE_HOME: tempHome,
      OTTIE_LISTEN: `0.0.0.0:${port}`,
      OPENAI_API_KEY: "",
      OTTIE_DICTATION_ENABLED: "0",
      OTTIE_VOICE_MODE_ENABLED: "0",
      OTTIE_LOG_FORMAT: "json",
    };
