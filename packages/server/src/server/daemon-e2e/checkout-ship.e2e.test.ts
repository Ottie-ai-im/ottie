...
function initGitRepo(repoDir: string): void {
  ...
  execSync("git config user.email 'ottie-test@example.com'", {
    cwd: repoDir,
    stdio: "pipe",
  });
  execSync("git config user.name 'Ottie Test'", {
    cwd: repoDir,
    stdio: "pipe",
  });
...
function createTempRepoName(): string {
  ...
  return `ottie-checkout-ship-${Date.now()}-${rand}`;
}
    ...
    async () => {
      ...
      try {
        ...
        const worktree = await createLegacyWorktreeForTest({
          ...
          ottieHome: ctx.daemon.ottieHome,
        });
        ...
        const worktreeList = await ctx.client.getOttieWorktreeList({
          cwd: repoDir,
        });
        ...
        const archiveResult = await ctx.client.archiveOttieWorktree({
          worktreePath: worktree.worktreePath,
        });
        expect(archiveResult.error).toBeNull();
        expect(archiveResult.success).toBe(true);

        const worktreeListAfter = await ctx.client.getOttieWorktreeList({
          cwd: repoDir,
        });
        expect(
