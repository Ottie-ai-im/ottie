...
interface CreateServiceOptions {
  ...
  listOttieWorktrees?: ReturnType<typeof vi.fn>;
...
function buildDefaultServiceDeps() {
  return {
    ...
    listOttieWorktrees: vi.fn(async () => []),
...
describe("WorkspaceGitServiceImpl D2 read methods", () => {
  ...
  test("listStashes cold-loads, warms, forces, and coalesces per cwd", async () => {
    let nowMs = 0;
    const stashOutput = "stash@{0}\u0000ottie-auto-stash: feature\n";
    ...
    const first = service.listStashes("/tmp/repo", { ottieOnly: true });
    const second = service.listStashes("/tmp/repo/.", { ottieOnly: true });
    await flushPromises();
    ...
    await expect(Promise.all([first, second])).resolves.toEqual([
      [{ index: 0, message: "ottie-auto-stash: feature", branch: "feature", isOttie: true }],
      [{ index: 0, message: "ottie-auto-stash: feature", branch: "feature", isOttie: true }],
    ]);

    nowMs = 1_000;
    await service.listStashes("/tmp/repo", { ottieOnly: true });
    expect(runGitCommand).toHaveBeenCalledTimes(1);

    await service.listStashes("/tmp/repo", { ottieOnly: true }, { force: true, reason: "test" });
    expect(runGitCommand).toHaveBeenCalledTimes(2);
  ...
  test("listWorktrees cold-loads, warms, forces, and coalesces per repo root", async () => {
    ...
    const listOttieWorktrees = vi.fn().mockResolvedValue(worktrees);
    const service = createService({
      listOttieWorktrees,
      now: () => new Date(nowMs),
    });
    ...
    expect(listOttieWorktrees).toHaveBeenCalledTimes(1);
    ...
    expect(listOttieWorktrees).toHaveBeenCalledTimes(2);
  ...
  test("listWorktrees shares one repo-root scoped read across sibling workspace cwds", async () => {
    ...
    const worktrees = [
      {
        path: join(tempDir, "ottie-home", "worktrees", "repo", "feature"),
        createdAt: "2026-04-12T00:00:00.000Z",
        branchName: "feature",
      },
    ];
    const listOttieWorktrees = vi.fn(async () => worktrees);
    const service = createService({
    ...
    try {
      ...
      expect(listOttieWorktrees).toHaveBeenCalledTimes(1);
      expect(listOttieWorktrees).toHaveBeenCalledWith({
        cwd: repoDir,
        ottieHome: "/tmp/ottie-test",
      });
    } finally {
  ...
  test("resolveRepoRemoteUrl reads remote URL through the snapshot cache", async () => {
    ...
    await expect(service.resolveRepoRemoteUrl("/tmp/repo")).resolves.toBe(
      "https://github.com/ottie/ottie.git",
    );
    nowMs = 1_000;
    await expect(service.resolveRepoRemoteUrl("/tmp/repo/.")).resolves.toBe(
      "https://github.com/ottie/ottie.git",
  ...
  test("getWorkspaceGitMetadata derives reconciliation metadata from the snapshot cache", async () => {
    ...
    await expect(
      ...
      gitRemote: "https://github.com/ottie/ottie.git",
      isWorktree: false,
      projectSlug: "ottie",
      repoRoot: "/tmp/repo",
      currentBranch: "feature/service-metadata",
