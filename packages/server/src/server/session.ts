...
import { isOttieDictationDebugEnabled } from "./agent/recordings-debug.js";
...
import {
  createOttieWorktree,
  type CreateOttieWorktreeInput,
  type CreateOttieWorktreeResult,
...
import {
  ...
  handleCreateOttieWorktreeRequest as handleCreateWorktreeRequest,
  handleOttieWorktreeArchiveRequest as handleWorktreeArchiveRequest,
  handleOttieWorktreeListRequest as handleWorktreeListRequest,
...
export class Session {
  ...
  private dispatchWorkspaceAndProjectMessage(
    ...
    switch (msg.type) {
      ...
      case "ottie_worktree_list_request":
        return this.handleOttieWorktreeListRequest(msg);
      case "ottie_worktree_archive_request":
        return this.handleOttieWorktreeArchiveRequest(msg);
      case "create_ottie_worktree_request":
        return this.handleCreateOttieWorktreeRequest(msg);
    ...
    return buildWorktreeAgentSessionConfig(
      {
        ...
        workspaceGitService: this.workspaceGitService,
        createOttieWorktree: (input, serviceOptions) =>
          this.createOttieWorktree(input, serviceOptions),
    ...
    try {
      ...
      if (
        ...
        return {
          ...
          body: "Automated PR generated by Ottie.",
        };
      }
      throw error;
    }
  ...
  private buildCheckoutStatusPayloadFromSnapshot({
    ...
    if (snapshot.git.isOttieOwnedWorktree) {
  ...
  private static readonly OTTIE_STASH_PREFIX = "ottie-auto-stash:";
  ...
  private async handleStashSaveRequest(
    ...
    try {
      ...
      const message = branchLabel
        ? `${Session.OTTIE_STASH_PREFIX} ${branchLabel}`
        : `${Session.OTTIE_STASH_PREFIX} unnamed`;
  ...
  private async handleStashListRequest(
    ...
    const { cwd, requestId } = msg;
    const ottieOnly = msg.ottieOnly !== false;
    try {
      const entries = await this.workspaceGitService.listStashes(cwd, { ottieOnly });
  ...
  private async handleOttieWorktreeListRequest(
    msg: Extract<SessionInboundMessage, { type: "ottie_worktree_list_request" }>,
  ): Promise<void> {
  ...
  private async handleOttieWorktreeArchiveRequest(
    msg: Extract<SessionInboundMessage, { type: "ottie_worktree_archive_request" }>,
  ): Promise<void> {
  ...
  private buildWorkspaceGitRuntimePayload(
    ...
    return {
      ...
      remoteUrl: snapshot.git.remoteUrl,
      isOttieOwnedWorktree: snapshot.git.isOttieOwnedWorktree,
      isDirty: snapshot.git.isDirty,
  ...
  private async describeCreatedWorktreeWorkspace(
    result: CreateOttieWorktreeResult,
  ): Promise<WorkspaceDescriptorPayload> {
  ...
  private async createOttieWorktree(
    input: CreateOttieWorktreeInput,
    options?: {
      resolveDefaultBranch?: (repoRoot: string) => Promise<string>;
    },
  ): Promise<CreateOttieWorktreeResult> {
    const coreDeps = createWorktreeCoreDeps(this.github);
    const result = await createOttieWorktree(input, {
      ...coreDeps,
  ...
  private async handleCreateOttieWorktreeRequest(
    request: Extract<SessionInboundMessage, { type: "create_ottie_worktree_request" }>,
  ): Promise<void> {
    return handleCreateWorktreeRequest(
      {
        ...
        emit: (message) => this.emit(message),
        createOttieWorktree: (input) => this.createOttieWorktree(input),
  ...
  private async handleArchiveWorkspaceRequest(
    ...
    try {
      ...
      if (existing.kind === "worktree") {
        throw new Error("Use worktree archive for Ottie worktrees");
      }
      const archivedAt = new Date().toISOString();
  ...
  private emit(msg: SessionOutboundMessage): void {
    ...
    if (
      ...
      (process.env.TTS_DEBUG_AUDIO_DIR || isOttieDictationDebugEnabled()) &&
      msg.payload.groupId &&
