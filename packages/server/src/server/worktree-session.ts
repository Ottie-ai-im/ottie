...
import type {
  CreateOttieWorktreeInput,
  CreateOttieWorktreeResult,
} from "./ottie-worktree-service.js";
import {
  archiveOttieWorktree,
  type ArchiveOttieWorktreeDependencies,
} from "./ottie-worktree-archive-service.js";
...
interface CreateOttieWorktreeInBackgroundDependencies {
  ottieHome?: string;
...
interface HandleCreateOttieWorktreeRequestDependencies {
  ottieHome?: string;
  describeWorkspaceRecord: (
    result: CreateOttieWorktreeResult,
  ) => Promise<WorkspaceDescriptorPayload>;
  emit: EmitSessionMessage;
  createOttieWorktree: (input: CreateOttieWorktreeInput) => Promise<CreateOttieWorktreeResult>;
...
export async function buildAgentSessionConfig(
  ...
  if (normalized.createWorktree) {
    ...
    const createdWorktree = await dependencies.createOttieWorktree(
      {
        cwd,
        ...
        attachments,
        runSetup: false,
        ottieHome: dependencies.ottieHome,
      },
      {
...
export async function handleOttieWorktreeListRequest(
  dependencies: {
    emit: EmitSessionMessage;
    ottieHome?: string;
    workspaceGitService: WorkspaceGitService;
...
export async function handleOttieWorktreeArchiveRequest(
  dependencies: Omit<
    ArchiveOttieWorktreeDependencies,
  ...
  try {
    if (!targetPath) {
      ...
      if (!match) {
        throw new Error(`Ottie worktree not found for branch ${msg.branchName}`);
      }
    ...
    const ownership = await isOttieOwnedWorktreeCwd(targetPath, {
      ottieHome: dependencies.ottieHome,
    });
    if (!ownership.allowed) {
      dependencies.emit({
        type: "ottie_worktree_archive_response",
        payload: {
          success: false,
          removedAgents: [],
          error: {
            code: "NOT_ALLOWED",
            message: "Worktree is not a Ottie-owned worktree",
          },
          requestId,
        },
      });
    ...
    // check already proved the path lives under $OTTIE_HOME/worktrees.
    repoRoot = ownership.repoRoot ?? repoRoot ?? null;

    const removedAgents = await archiveOttieWorktree(dependencies, {
      targetPath,
      repoRoot,
...
export async function handleCreateOttieWorktreeRequest(
  dependencies: HandleCreateOttieWorktreeRequestDependencies,
  ...
  try {
    ...
    dependencies.emit({
      type: "create_ottie_worktree_response",
      payload: {
...
export async function runWorktreeSetupInBackground(
  dependencies: CreateOttieWorktreeInBackgroundDependencies,
  options: {
    requestCwd: string;
    repoRoot: string;
    workspaceId: string;
