...
function getPidFilePath(ottieHome: string): string {
  return join(ottieHome, "ottie.pid");
}
...
export async function acquirePidLock(
  ...
  const pidPath = getPidFilePath(ottieHome);

  // Ensure ottieHome directory exists
  ...
  if (existingLock) {
    if (isPidRunning(existingLock.pid)) {
      ...
      throw new PidLockError(
        `Another Ottie daemon is already running (PID ${existingLock.pid}, started ${existingLock.startedAt})`,
        existingLock,
      );
    }
  ...
  const lockInfo: PidLockInfo = {
    ...
    listen,
    ...(process.env.OTTIE_DESKTOP_MANAGED === "1" ? { desktopManaged: true } : {}),
  };
  ...
  try {
    ...
    if ((err as NodeJS.ErrnoException).code === "EEXIST") {
      ...
      try {
        ...
        throw new PidLockError(
          `Another Ottie daemon is already running (PID ${raceLock.pid})`,
          raceLock,
        );
      } catch (innerErr) {
...
export async function getPidLockInfo(ottieHome: string): Promise<PidLockInfo | null> {
  const pidPath = getPidFilePath(ottieHome);
  try {
    const content = await readFile(pidPath, "utf-8");
...
export async function isLocked(
  ...
  const info = await getPidLockInfo(ottieHome);
  if (!info) {
    return { locked: false };
  }
