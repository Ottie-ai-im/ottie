...
export class DaemonConfigStore {
  ...
  constructor(ottieHome: string, initial: MutableDaemonConfig, logger?: LoggerLike) {
  ...
  private persistConfig(config: MutableDaemonConfig): void {
    const persisted = loadPersistedConfig(this.ottieHome, this.logger);
    const nextPersisted = mergeMutableConfigIntoPersistedConfig({
      persisted,
      mutable: config,
    });
    savePersistedConfig(this.ottieHome, nextPersisted, this.logger);
  }
}
