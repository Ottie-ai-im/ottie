...
function createGitRepo(): { tempDir: string; repoDir: string; ottieHome: string } {
  ...
  return { tempDir, repoDir, ottieHome };
}

function createGitRepoWithDevBranch(): { tempDir: string; repoDir: string; ottieHome: string } {
  const { tempDir, repoDir, ottieHome } = createGitRepo();
...
function createGitHubPrRemoteRepo(): { tempDir: string; repoDir: string; ottieHome: string } {
  const { tempDir, repoDir, ottieHome } = createGitRepo();
...
function createForkGitHubPrRemoteRepo(): {
  ...
  return { tempDir, repoDir, headRemoteDir, ottieHome };
}
...
describe.skipIf(process.platform === "win32")("createWorktreeCore", () => {
  ...
  test("checks out the legacy RPC GitHub PR attachment branch", async () => {
    const { tempDir, repoDir, ottieHome } = createGitHubPrRemoteRepo();
    cleanupPaths.push(tempDir);
  ...
  test("branches off an explicit refName base", async () => {
    const { tempDir, repoDir, ottieHome } = createGitRepoWithDevBranch();
    cleanupPaths.push(tempDir);
  ...
  test("checks out a fork PR whose head branch collides with local main", async () => {
    const { tempDir, repoDir, headRemoteDir, ottieHome } = createForkGitHubPrRemoteRepo();
    cleanupPaths.push(tempDir);
    const github = {
  ...
  test("uses a unique local branch when the same fork PR branch already exists", async () => {
    ...
    expect(
      execSync("git config --get remote.ottie-pr-526.push", {
  ...
  test("reuses an existing branch-off worktree for the same slug", async () => {
    ...
    const first = await createCoreWorktree(
      { cwd: repoDir, worktreeSlug: "reused-worktree", ottieHome, runSetup: false },
      deps,
    );
    const second = await createCoreWorktree(
      { cwd: repoDir, worktreeSlug: "reused-worktree", ottieHome, runSetup: false },
...
describe("resolveWorktreeRepoRoot", () => {
  test("resolves repository roots through the workspace git service", async () => {
    ...
    await expect(
      resolveWorktreeRepoRoot(
        { cwd: "/tmp/main-repo/worktrees/feature", ottieHome: "/tmp/ottie-home" },
        workspaceGitService,
      ),
