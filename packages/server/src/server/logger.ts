...
function resolveFilePath(ottieHome: string, configuredPath: string | undefined): string {
  const fallback = path.join(ottieHome, DEFAULT_DAEMON_LOG_FILENAME);
  if (!configuredPath) {
    return fallback;
  }
  ...
  return path.resolve(ottieHome, configuredPath);
}
...
function resolveConfiguredOttieHome(options: ResolveLogConfigOptions | undefined): string {
  if (options?.ottieHome) {
    return options.ottieHome;
  }
  return resolveOttieHome(options?.env ?? process.env);
}
...
function resolveRotateConfig(
  ...
  return {
    maxSize:
      env.OTTIE_LOG_FILE_ROTATE_SIZE?.trim() ||
      persistedLog?.file?.rotate?.maxSize ||
      DEFAULT_FILE_ROTATE_SIZE,
    maxFiles:
      parsePositiveInteger(env.OTTIE_LOG_FILE_ROTATE_COUNT) ??
...
export function resolveLogConfig(
  ...
  const ottieHome = resolveConfiguredOttieHome(options);
  ...
  const filePath = resolveFilePath(ottieHome, env.OTTIE_LOG_FILE_PATH ?? persistedLog?.file?.path);
