...
describe("codex tool-call mapper", () => {
  ...
  it("normalizes codex ottie speak mcp calls and extracts spoken text", () => {
    const item = mapCodexToolCallFromThreadItem({
      type: "mcpToolCall",
  ...
  it("normalizes codex ottie_voice.speak mcp calls and extracts spoken text", () => {
    const item = mapCodexToolCallFromThreadItem({
      type: "mcpToolCall",
      ...
      server: "ottie_voice",
      tool: "speak",
      arguments: { text: "Voice response from Codex via ottie_voice." },
      result: { ok: true },
    });
    ...
    expect(item?.detail).toEqual({
      type: "unknown",
      input: "Voice response from Codex via ottie_voice.",
      output: null,
    });
  });

  it("normalizes codex ottie speak rollout names and extracts spoken text", () => {
    const item = expectMapped(
      mapCodexRolloutToolCall({
