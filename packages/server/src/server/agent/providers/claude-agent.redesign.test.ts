...
test("logs redacted query summary and never leaks sentinel secrets", async () => {
  const envSecret = "OTTIE_ENV_SENTINEL_SECRET";
  const runtimeSecret = "OTTIE_RUNTIME_SENTINEL_SECRET";
  const systemSecret = "OTTIE_SYSTEM_PROMPT_SENTINEL_SECRET";
  const previousEnv = process.env.OTTIE_TEST_SENTINEL_SECRET;
  process.env.OTTIE_TEST_SENTINEL_SECRET = envSecret;
  ...
  const client = new ClaudeAgentClient({
    ...
    runtimeSettings: {
      env: {
        OTTIE_RUNTIME_SENTINEL_SECRET: runtimeSecret,
      },
    },
  });
  ...
  try {
    ...
    if (previousEnv === undefined) {
      delete process.env.OTTIE_TEST_SENTINEL_SECRET;
    } else {
      process.env.OTTIE_TEST_SENTINEL_SECRET = previousEnv;
    }
  }
});
