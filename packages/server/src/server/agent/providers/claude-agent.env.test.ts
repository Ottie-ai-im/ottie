...
describe("Claude agent env", () => {
  test("forwards launch-context env through Claude process env", async () => {
    ...
    const launchContext: AgentLaunchContext = {
      env: {
        OTTIE_AGENT_ID: "00000000-0000-4000-8000-000000000201",
        OTTIE_TEST_FLAG: "launch-value",
      },
    };
    ...
    try {
      ...
      expect(capturedEnv?.OTTIE_AGENT_ID).toBe(launchContext.env?.OTTIE_AGENT_ID);
      expect(capturedEnv?.OTTIE_TEST_FLAG).toBe(launchContext.env?.OTTIE_TEST_FLAG);
    } finally {
      await session.close();
    }
  });
  ...
  test("forwards launch-context env through Claude resume env", async () => {
    ...
    const launchContext: AgentLaunchContext = {
      env: {
        OTTIE_AGENT_ID: "00000000-0000-4000-8000-000000000202",
        OTTIE_TEST_FLAG: "resume-launch-value",
      },
    };
    const queryFactory = vi.fn(
      ({ options }: { options: { env?: Record<string, string | undefined> } }) => {
