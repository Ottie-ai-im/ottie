...
describe("Codex app-server provider", () => {
  ...
  test("builds app-server env from launch-context env overrides", () => {
    const launchContext: AgentLaunchContext = {
      env: {
        OTTIE_AGENT_ID: "00000000-0000-4000-8000-000000000301",
        OTTIE_TEST_FLAG: "codex-launch-value",
      },
    };
    const env = __codexAppServerInternals.buildCodexAppServerEnv(
      {
        env: {
          OTTIE_AGENT_ID: "runtime-value",
          OTTIE_TEST_FLAG: "runtime-test-value",
        },
      },
      launchContext.env,
    );

    expect(env.OTTIE_AGENT_ID).toBe(launchContext.env?.OTTIE_AGENT_ID);
    expect(env.OTTIE_TEST_FLAG).toBe(launchContext.env?.OTTIE_TEST_FLAG);
  });
