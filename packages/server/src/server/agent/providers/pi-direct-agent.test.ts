...
describe("PiDirectAgentClient", () => {
  test("lists only Pi models with configured auth", async () => {
    ...
    const models = await client.listModels({ cwd: "/tmp/ottie-pi-test", force: false });
  ...
  test("loads project extensions before listing available models", async () => {
    const testRoot = await mkdtemp(join(tmpdir(), "ottie-pi-extension-"));
...
export default function(pi) {
  pi.registerProvider("ottie-dummy", {
    baseUrl: "https://example.invalid/v1",
    apiKey: "ottie-test-key",
...
`,
      ...
      expect(models.map((model) => model.id)).toContain("ottie-dummy/extension-model");
    } finally {
  ...
  test("creates sessions with project extension models and exposes extension commands", async () => {
    const testRoot = await mkdtemp(join(tmpdir(), "ottie-pi-extension-session-"));
    const previousAgentDir = process.env.PI_CODING_AGENT_DIR;
...
`,
      ...
      const session = await client.createSession({
        provider: "pi",
        cwd,
        model: "ottie-dummy/extension-model",
      });
