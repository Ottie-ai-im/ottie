...
const JSON_ARG = '{"mcpServers":{"ottie":{"type":"http","url":"http://127.0.0.1:6767/mcp"}}}';
const tempDirs: string[] = [];

function makeFixture(binaryName: string, expectedArgs: string[]) {
  const root = mkdtempSync(path.join(tmpdir(), `ottie ${binaryName} launch `));
  tempDirs.push(root);

  const fakeDaemonNode = path.join(root, "Fake Ottie.exe");
...
const expected = JSON.parse(process.env.OTTIE_EXPECTED_ARGV_JSON);
...
async function runProviderFixture(params: {
  ...
  const child = spawnProcess(params.command, params.args, {
    env: {
      ...process.env,
      OTTIE_EXPECTED_ARGV_JSON: JSON.stringify(params.expectedArgs),
    },
