  ...
  test("custom agents defined in opencode.json appear in available modes", async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        agent: {
          "ottie-test-custom": {
            description: "Custom agent defined for Ottie integration test",
            mode: "primary",
          },
        },
      }),
    ...
    const custom = modes.find((mode) => mode.id === "ottie-test-custom");
    expect(custom).toBeDefined();
    expect(custom!.label).toBe("Ottie-test-custom");
    expect(custom!.description).toBe("Custom agent defined for Ottie integration test");
    ...
    const parts = __openCodeInternals.buildOpenCodePromptParts([
      {
        ...
        url: "https://github.com/ottie/ottie/issues/55",
        body: "Issue body",
      },
    ]);
