...
async function canRenameBranch(
  ...
  if (!snapshot.git.isGit || !snapshot.git.isOttieOwnedWorktree) {
    return false;
  }
...
export async function determineAgentMetadataNeeds(
  options: Pick<
    ...
    "initialPrompt" | "explicitTitle" | "cwd" | "ottieHome" | "deps"
  >,
...
async function applyGeneratedBranchRename(params: {
  ...
  if (!snapshot.git.isGit || !snapshot.git.isOttieOwnedWorktree || !snapshot.git.currentBranch) {
    return;
  }
