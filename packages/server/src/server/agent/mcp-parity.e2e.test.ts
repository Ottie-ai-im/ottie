...
import { createTestOttieDaemon, type TestOttieDaemon } from "../test-utils/ottie-daemon.js";
...
let tempRoot: string;
let daemonHandle: TestOttieDaemon;
...
beforeAll(async () => {
  ...
  daemonHandle = await createTestOttieDaemon();
...
describe("Suite A: Core Fixes", () => {
  ...
  test("create_agent with callerAgentId sets ottie.parent-agent-id label", async () => {
    ...
    try {
      ...
      expect(snapshot?.labels).toMatchObject({
        "ottie.parent-agent-id": parentAgentId,
      });
    } finally {
  ...
  test("agentManager.createAgent injects ottie MCP using the daemon listen target", async () => {
    let agentId: string | null = null;
    try {
