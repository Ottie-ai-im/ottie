...
export function isOttieToolName(name: string): boolean {
  const normalized = normalizeToolName(name);
  ...
  if (normalized.includes("__")) {
    ...
    return (
      segments.length >= 3 &&
      segments[0] === "mcp" &&
      (segments[1] === "ottie" || segments[1]!.startsWith("ottie_"))
    );
  }
  if (normalized.includes(".")) {
    const firstSegment = normalized.split(".")[0]!;
    return firstSegment === "ottie" || firstSegment.startsWith("ottie_");
  }
  return false;
}

export function getOttieToolLeafName(name: string): string | null {
  const normalized = normalizeToolName(name);
  if (normalized.includes("__")) {
    const segments = normalized.split("__").filter((s) => s.length > 0);
    if (
  ...
  if (normalized.includes(".")) {
    const firstSegment = normalized.split(".")[0]!;
    if (firstSegment === "ottie" || firstSegment.startsWith("ottie_")) {
