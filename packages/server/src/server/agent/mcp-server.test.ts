...
function createOttieWorktreeForMcpTest(options: {
  ottieHome: string;
  broadcasts: string[];
  ...
  return async (input, serviceOptions) => {
    ...
    const result = await createOttieWorktreeService(input, {
      ...coreDeps,
...
describe("create_agent MCP tool", () => {
  ...
  it("registers and broadcasts a workspace when create_agent creates a worktree", async () => {
    const { agentManager, agentStorage, spies } = createTestDeps();
    const tempDir = await mkdtemp(join(tmpdir(), "ottie-mcp-worktree-"));
    const repoDir = join(tempDir, "repo");
    const ottieHome = join(tempDir, ".ottie");
    ...
    try {
      ...
      const server = await createAgentMcpServer({
        ...
        agentStorage,
        ottieHome,
        createOttieWorktree: createOttieWorktreeForMcpTest({
          ottieHome,
          broadcasts,
          createdWorkspaceIds,
        }),
  ...
  it("registers and broadcasts a workspace when create_worktree creates a worktree", async () => {
    const { agentManager, agentStorage } = createTestDeps();
    const tempDir = await mkdtemp(join(tmpdir(), "ottie-mcp-create-worktree-"));
    const repoDir = join(tempDir, "repo");
    const ottieHome = join(tempDir, ".ottie");
    const broadcasts: string[] = [];
    ...
    try {
      ...
      const server = await createAgentMcpServer({
        ...
        agentStorage,
        ottieHome,
        createOttieWorktree: createOttieWorktreeForMcpTest({ ottieHome, broadcasts }),
  ...
  it("forces a workspace git snapshot refresh when archive_worktree deletes a worktree", async () => {
    const { agentManager, agentStorage } = createTestDeps();
    const tempDir = await mkdtemp(join(tmpdir(), "ottie-mcp-archive-worktree-"));
    const repoDir = join(tempDir, "repo");
    const ottieHome = join(tempDir, ".ottie");
  ...
  it("routes list_worktrees through WorkspaceGitService", async () => {
    ...
    const workspaceGitService = {
      ...
      listWorktrees: vi.fn(async () => [
        {
          path: "/tmp/ottie/worktrees/repo/feature",
          branchName: "feature",
  ...
  it("allows caller agents to override cwd and applies caller context labels", async () => {
    const { agentManager, agentStorage, spies } = createTestDeps();
    const baseDir = await mkdtemp(join(tmpdir(), "ottie-mcp-test-"));
    ...
    expect(spies.agentManager.createAgent).toHaveBeenCalledWith(
      ...
      {
        labels: {
          "ottie.parent-agent-id": "voice-agent",
          source: "voice",
        },
      },
    );
