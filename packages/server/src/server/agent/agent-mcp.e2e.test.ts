...
import { createOttieDaemon, type OttieDaemonConfig } from "../bootstrap.js";
...
describe("agent MCP end-to-end (offline)", () => {
  test("create_agent runs initial prompt and affects filesystem", async () => {
    const ottieHome = await mkdtemp(path.join(os.tmpdir(), "ottie-home-"));
    const staticDir = await mkdtemp(path.join(os.tmpdir(), "ottie-static-"));
    const agentCwd = await mkdtemp(path.join(os.tmpdir(), "ottie-agent-cwd-"));
    const port = await getAvailablePort();

    const daemonConfig: OttieDaemonConfig = {
    ...
    const daemon = await createOttieDaemon(daemonConfig, pino({ level: "silent" }));
    await daemon.start();
  ...
  test("create_agent auto-injects ottie MCP by default and can be disabled", async () => {
    const ottieHome = await mkdtemp(path.join(os.tmpdir(), "ottie-home-"));
    const staticDir = await mkdtemp(path.join(os.tmpdir(), "ottie-static-"));
    const agentCwd = await mkdtemp(path.join(os.tmpdir(), "ottie-agent-cwd-"));
    const port = await getAvailablePort();
    ...
    const disabledOttieHome = await mkdtemp(path.join(os.tmpdir(), "ottie-home-disabled-"));
    const disabledStaticDir = await mkdtemp(path.join(os.tmpdir(), "ottie-static-disabled-"));
    const disabledAgentCwd = await mkdtemp(path.join(os.tmpdir(), "ottie-agent-cwd-disabled-"));
    const disabledPort = await getAvailablePort();
    const disabledDaemonConfig: OttieDaemonConfig = {
      listen: `127.0.0.1:${disabledPort}`,
      ottieHome: disabledOttieHome,
      corsAllowedOrigins: [],
      hostnames: true,
      mcpEnabled: true,
      ...
      mcpDebug: false,
      agentClients: createTestAgentClients(),
      agentStoragePath: path.join(disabledOttieHome, "agents"),
    };
    const disabledDaemon = await createOttieDaemon(disabledDaemonConfig, pino({ level: "silent" }));
    ...
    try {
      ...
      expect(disabledAgent?.config.mcpServers?.ottie).toBeUndefined();
    } finally {
      if (agentId) {
        await client.callTool({ name: "kill_agent", args: { agentId } });
      }
      ...
      await rm(disabledOttieHome, { recursive: true, force: true });
  ...
  test("create_agent with worktree is async and boots terminals only after setup success", async () => {
    const ottieHome = await mkdtemp(path.join(os.tmpdir(), "ottie-home-"));
    const staticDir = await mkdtemp(path.join(os.tmpdir(), "ottie-static-"));
    const repoRoot = await mkdtemp(path.join(os.tmpdir(), "ottie-worktree-repo-"));
    const port = await getAvailablePort();
    ...
    try {
      ...
      const setupCommand =
        'while [ ! -f "$OTTIE_WORKTREE_PATH/allow-setup" ]; do sleep 0.05; done; echo "done" > "$OTTIE_WORKTREE_PATH/setup-done.txt"';
      await writeFile(
        path.join(repoRoot, "ottie.json"),
        JSON.stringify({
          worktree: {
            setup: [setupCommand],
            terminals: [
      ...
      execSync("git add ottie.json", { cwd: repoRoot, stdio: "pipe" });
