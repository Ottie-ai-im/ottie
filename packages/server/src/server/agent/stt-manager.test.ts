...
describe("STTManager", () => {
  ...
  it("uses streaming segmentation for batch transcription and concatenates segment finals", async () => {
    const original = process.env.OTTIE_STT_BATCH_COMMIT_EVERY_SECONDS;
    process.env.OTTIE_STT_BATCH_COMMIT_EVERY_SECONDS = "1";
    ...
    try {
      ...
      if (original === undefined) {
        delete process.env.OTTIE_STT_BATCH_COMMIT_EVERY_SECONDS;
      } else {
        process.env.OTTIE_STT_BATCH_COMMIT_EVERY_SECONDS = original;
      }
    }
  });
});
