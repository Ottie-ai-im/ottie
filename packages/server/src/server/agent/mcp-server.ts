...
import { getOttieWorktreesRoot, type WorktreeConfig } from "../../utils/worktree.js";
import {
...
export interface AgentMcpServerOptions {
  ...
  archiveWorkspaceRecord?: ArchiveOttieWorktreeDependencies["archiveWorkspaceRecord"];
  emitWorkspaceUpdatesForCwds?: ArchiveOttieWorktreeDependencies["emitWorkspaceUpdatesForCwds"];
  emitSessionMessage?: ArchiveOttieWorktreeDependencies["emit"];
  createOttieWorktree?: CreateOttieWorktreeFn;
  ottieHome?: string;
  /**
...
export async function createAgentMcpServer(options: AgentMcpServerOptions): Promise<McpServer> {
  ...
  const resolveTopLevelCreateAgentArgs = async (
    ...
    if (worktreeName) {
      ...
      const createdWorktree = await createMcpWorktree({
        ...
        createOttieWorktree: options.createOttieWorktree,
  ...
  server.registerTool(
    ...
    async (args: unknown) => {
      ...
      const mergedLabels = {
        ...(callerAgentId ? { "ottie.parent-agent-id": callerAgentId } : {}),
  ...
  server.registerTool(
    "list_worktrees",
    {
      ...
      description: "List Ottie-managed git worktrees for a repository.",
      inputSchema: {
        cwd: z
          .string()
          .optional()
  ...
  server.registerTool(
    "create_worktree",
    {
      ...
      description: "Create a Ottie-managed git worktree.",
      inputSchema: {
        cwd: z
          .string()
          .optional()
  ...
  server.registerTool(
    "archive_worktree",
    {
      ...
      description: "Delete a Ottie-managed git worktree.",
      inputSchema: {
        cwd: z
          .string()
          .optional()
    ...
    async ({ cwd, worktreePath, worktreeSlug }) => {
      ...
      const targetPath =
        worktreePath ??
        join(await getOttieWorktreesRoot(repoRoot, options.ottieHome), worktreeSlug!);
...
interface CreateMcpWorktreeOptions {
  ...
  createOttieWorktree: CreateOttieWorktreeFn | undefined;
...
async function createMcpWorktree(
  ...
  try {
    if (!options.createOttieWorktree) {
      throw new Error("Ottie worktree service is not configured");
    }
    const result = await options.createOttieWorktree(options.input, {
