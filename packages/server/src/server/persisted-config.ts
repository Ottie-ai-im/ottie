...
function getConfigPath(ottieHome: string): string {
  return path.join(ottieHome, CONFIG_FILENAME);
}
...
export function loadPersistedConfig(ottieHome: string, logger?: LoggerLike): PersistedConfig {
  const log = getLogger(logger);
  const configPath = getConfigPath(ottieHome);
