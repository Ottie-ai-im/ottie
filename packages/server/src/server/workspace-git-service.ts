...
import { listOttieWorktrees, type OttieWorktreeInfo } from "../utils/worktree.js";
...
const linuxWatchReaddirConcurrency =
  parseInt(process.env.OTTIE_LINUX_WATCH_READDIR_CONCURRENCY ?? "16", 10) || 16;
...
export type WorkspaceGitWorktreeInfo = OttieWorktreeInfo;
...
interface WorkspaceGitServiceDependencies {
  ...
  listOttieWorktrees: typeof listOttieWorktrees;
...
export class WorkspaceGitServiceImpl implements WorkspaceGitService {
  ...
  getCheckoutDiff(
    ...
    return this.readAuxiliaryCache(this.checkoutDiffCache, key, readOptions, () =>
      this.deps.getCheckoutDiff(normalizedCwd, normalizedOptions, { ottieHome: this.ottieHome }),
    );
  }
  ...
  listStashes(
    ...
    const normalizedCwd = normalizeWorkspaceId(cwd);
    const ottieOnly = options?.ottieOnly !== false;
    const key = JSON.stringify(["stashes", normalizedCwd, ottieOnly]);
    return this.readAuxiliaryCache(this.stashListCache, key, readOptions, async () => {
      ...
      return parseWorkspaceGitStashList(stdout, { ottieOnly });
    });
  }
  ...
  async resolveRepoRoot(cwd: string, options?: WorkspaceGitReadOptions): Promise<string> {
    ...
    return snapshot.git.isOttieOwnedWorktree
...
async function loadWorkspaceGitRuntimeSnapshot(
  ...
  return {
    ...
    git: {
      ...
      mainRepoRoot: checkoutStatus.isOttieOwnedWorktree ? checkoutStatus.mainRepoRoot : null,
      currentBranch: checkoutStatus.currentBranch,
      remoteUrl: checkoutStatus.remoteUrl,
      isOttieOwnedWorktree: checkoutStatus.isOttieOwnedWorktree,
...
function parseWorkspaceGitStashList(
  ...
  for (const line of lines) {
    ...
    const prefix = "ottie-auto-stash:";
    const prefixIdx = subject.indexOf(prefix);
    const isOttie = prefixIdx >= 0;
    const branch = isOttie ? subject.slice(prefixIdx + prefix.length).trim() || null : null;

    if (options.ottieOnly && !isOttie) {
      continue;
    }

    entries.push({ index, message: subject, branch, isOttie });
  }
