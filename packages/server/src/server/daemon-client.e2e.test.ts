...
const localModelsDir =
  process.env.OTTIE_LOCAL_MODELS_DIR ?? path.join(homedir(), ".ottie", "models", "local-speech");
  ...
  if (hasLocalSpeech) {
    return {
      ...
      local: {
        ...
        models: {
          dictationStt:
            process.env.OTTIE_DICTATION_LOCAL_STT_MODEL ?? "zipformer-bilingual-zh-en-2023-02-20",
          voiceStt:
            process.env.OTTIE_VOICE_LOCAL_STT_MODEL ?? "zipformer-bilingual-zh-en-2023-02-20",
          voiceTts: process.env.OTTIE_VOICE_LOCAL_TTS_MODEL ?? "kitten-nano-en-v0_1-fp16",
        },
      },
    };
  }
...
test("returns home-scoped directory suggestions", async () => {
  const insideHomeDir = mkdtempSync(path.join(homedir(), "ottie-dir-suggestion-"));
  const outsideHomeDir = mkdtempSync(path.join(tmpdir(), "ottie-dir-suggestion-outside-"));
