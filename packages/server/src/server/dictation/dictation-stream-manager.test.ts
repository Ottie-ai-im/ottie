...
describe("DictationStreamManager (finish buffer-too-small tolerance)", () => {
  const env = {
    dictationDebug: process.env.OTTIE_DICTATION_DEBUG,
  };

  beforeEach(() => {
    vi.useFakeTimers();
    process.env.OTTIE_DICTATION_DEBUG = "false";
  });

  afterEach(() => {
    vi.useRealTimers();
    process.env.OTTIE_DICTATION_DEBUG = env.dictationDebug;
  });
...
describe("DictationStreamManager (provider-agnostic provider)", () => {
  ...
  it("auto-commits while streaming and assembles final transcript in segment order", async () => {
    const originalDebug = process.env.OTTIE_DICTATION_DEBUG;
    process.env.OTTIE_DICTATION_DEBUG = "false";
    ...
    try {
      ...
      if (originalDebug === undefined) {
        delete process.env.OTTIE_DICTATION_DEBUG;
      } else {
        process.env.OTTIE_DICTATION_DEBUG = originalDebug;
      }
    }
  });
  ...
  it("drops dangling uncommitted non-final transcripts when finishing after silence tail clear", async () => {
    vi.useFakeTimers();
    const previousDebug = process.env.OTTIE_DICTATION_DEBUG;
    process.env.OTTIE_DICTATION_DEBUG = "false";
    try {
      const session = new FakeRealtimeSession();
      const emitted: Array<{ type: string; payload: unknown }> = [];
      ...
      process.env.OTTIE_DICTATION_DEBUG = previousDebug;
      vi.useRealTimers();
    }
  });
});
