...
interface SessionHandlerInternals {
  ...
  createOttieWorktree(params: unknown): Promise<unknown>;
...
const ottieWorktreeServiceMocks = vi.hoisted(() => ({
  createOttieWorktree: vi.fn(),
}));
...
vi.mock("./ottie-worktree-service.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("./ottie-worktree-service.js")>();
  return {
    ...actual,
    createOttieWorktree: ottieWorktreeServiceMocks.createOttieWorktree,
  };
});
...
function createWorkspaceGitSnapshot(
  ...
  return {
    ...
    git: {
      ...
      remoteUrl: "https://github.com/ottie/ottie.git",
      ...
      checks: [
        {
          name: "typecheck",
          status: "success",
          url: "https://github.com/ottie/ottie/actions/runs/1",
          workflow: "CI",
          duration: "1m 20s",
        },
      ],
...
describe("session checkout pull request creation", () => {
  ...
  test("forces workspace git and GitHub refresh after creating a pull request", async () => {
    ...
    checkoutGitMocks.createPullRequest.mockResolvedValue({
      url: "https://github.com/ottie/ottie/pull/2",
      number: 2,
    });
    const session = createSessionForTest({ github, workspaceGitService, messages });
...
describe("session branch validation", () => {
  ...
  test("does not validate tags as branches", async () => {
    const tempDir = mkdtempSync(join(tmpdir(), "ottie-session-branch-validation-"));
...
describe("session stash list handling", () => {
  test("lists stashes through the workspace git service", async () => {
    ...
    const entries = [
      {
        index: 0,
        message: "ottie-auto-stash: feature",
        branch: "feature",
        isOttie: true,
      },
    ];
...
describe("session ottie worktree creation handling", () => {
  test("forces workspace git refreshes for the source repo and created worktree", async () => {
    const workspaceGitService = { getSnapshot: vi.fn().mockResolvedValue({}) };
    const session = createSessionForTest({ workspaceGitService });
    ottieWorktreeServiceMocks.createOttieWorktree.mockResolvedValue({
      repoRoot: "/tmp/repo",
      worktree: {
        branchName: "feature/new-worktree",
        worktreePath: "/tmp/ottie/worktrees/new-worktree",
      },
      workspace: {
        workspaceId: "workspace-new-worktree",
        projectId: "project-repo",
        cwd: "/tmp/ottie/worktrees/new-worktree",
        kind: "worktree",
    ...
    await asSessionInternals(session).createOttieWorktree({
      cwd: "/tmp/repo",
    ...
    expect(workspaceGitService.getSnapshot).toHaveBeenCalledWith(
      "/tmp/ottie/worktrees/new-worktree",
      {
        force: true,
        reason: "create-worktree",
      },
  ...
  test("passes service-owned git metadata into workspace script spawning", async () => {
    ...
    const workspaceGitService = {
      ...
      getWorkspaceGitMetadata: vi.fn().mockResolvedValue({
        projectKind: "git",
        projectDisplayName: "ottie/ottie",
...
describe("session pull request timeline handling", () => {
        ...
        items: [
          {
            ...
            number: 42,
            title: "Ship search",
            url: "https://github.com/ottie/ottie/pull/42",
            state: "OPEN",
            body: null,
            labels: [],
            baseRefName: "main",
        ...
        items: [
          {
            ...
            url: "https://github.com/ottie/ottie/pull/42#pullrequestreview-1",
  ...
  test.each([
    { prNumber: 0, repoOwner: "ottie", repoName: "ottie" },
    { prNumber: -1, repoOwner: "ottie", repoName: "ottie" },
    { prNumber: 42, repoOwner: "get ottie", repoName: "ottie" },
    { prNumber: 42, repoOwner: "ottie/cli", repoName: "ottie" },
    { prNumber: 42, repoOwner: "get$ottie", repoName: "ottie" },
    { prNumber: 42, repoOwner: "ottie", repoName: "pa seo" },
    { prNumber: 42, repoOwner: "ottie", repoName: "ottie/app" },
    { prNumber: 42, repoOwner: "ottie", repoName: "ottie!" },
