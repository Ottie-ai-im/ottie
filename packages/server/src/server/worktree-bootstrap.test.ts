...
describe("runAsyncWorktreeBootstrap", () => {
  ...
  it("truncates each command output to 64kb in the middle", async () => {
    ...
    const persistedSetupItem = persisted.find(
      (item): item is Extract<AgentTimelineItem, { type: "tool_call" }> =>
        item.type === "tool_call" && item.name === "ottie_worktree_setup",
    );
  ...
  it("shares the same worktree runtime port across setup and bootstrap terminals", async () => {
    writeFileSync(
      ...
      JSON.stringify({
        worktree: {
          setup: ['echo "$OTTIE_WORKTREE_PORT" > setup-port.txt'],
          terminals: [
            {
              name: "Port Terminal",
              command: "true",
    ...
    expect(registeredEnvs[0]?.env.OTTIE_WORKTREE_PORT).toBe(setupPort);
    expect(createTerminalEnvs.length).toBeGreaterThan(0);
    expect(createTerminalEnvs[0]?.OTTIE_WORKTREE_PORT).toBe(setupPort);

    const terminalToolCall = persisted.find(
      (item): item is Extract<AgentTimelineItem, { type: "tool_call" }> =>
        item.type === "tool_call" &&
        item.name === "ottie_worktree_terminals" &&
  ...
  function assertServiceTerminalCallSelfEnv(params: {
    ...
    expect(createTerminalCalls[0]?.env?.OTTIE_PORT).toEqual(expect.any(String));
    expect(createTerminalCalls[0]?.env?.HOST).toBe("127.0.0.1");
    expect(createTerminalCalls[0]?.env?.OTTIE_URL).toBe(
      "http://api.feature-socket-service.repo.localhost:6767",
    );
    expect(createTerminalCalls[0]?.env?.OTTIE_SERVICE_API_PORT).toBe(
      createTerminalCalls[0]?.env?.OTTIE_PORT,
    );
    expect(createTerminalCalls[0]?.env?.OTTIE_SERVICE_API_URL).toBe(
      "http://api.feature-socket-service.repo.localhost:6767",
    );
  }
  ...
  async function assertServiceTerminalCallPeerEnv(params: {
    ...
    expect(createTerminalCalls[0]?.env?.OTTIE_SERVICE_APP_SERVER_PORT).toBe(
      String(plannedAppServerPort),
    );
    expect(createTerminalCalls[0]?.env?.OTTIE_SERVICE_APP_SERVER_URL).toBe(
  ...
  function commitOttieScripts(
    scripts: Record<string, { command: string; type?: "script" | "service" }>,
    message = "add script config",
  ): void {
    writeFileSync(join(repoDir, "ottie.json"), JSON.stringify({ scripts }));
    execSync("git add ottie.json", { cwd: repoDir, stdio: "pipe" });
  ...
  it("refreshes a stopped service port on respawn and updates the route", async () => {
    ...
    expect(createTerminalCalls[2]?.env?.OTTIE_SERVICE_WORKER_PORT).toBe(String(workerPort));
  ...
  it("fails normalized service env name collisions before terminal creation", async () => {
    ...
    expect(createTerminalCalls).toHaveLength(1);
    expect(createTerminalCalls[0]?.env).toHaveProperty("OTTIE_SERVICE_APP_SERVER_PORT");
    expect(createTerminalCalls[0]?.env).toHaveProperty("OTTIE_SERVICE_WORKER_PORT");
  });
  ...
  it("injects real peer service env into terminal-backed services", async () => {
    ...
    expect(apiEnv.OTTIE_SERVICE_API_URL).toBe("http://api.feature-peer-env.repo.localhost:6767");
    expect(apiEnv.OTTIE_SERVICE_WEB_URL).toBe("http://web.feature-peer-env.repo.localhost:6767");
    expect(apiEnv.OTTIE_SERVICE_API_PORT).toEqual(expect.stringMatching(/^\d+$/));
    expect(apiEnv.OTTIE_SERVICE_WEB_PORT).toEqual(expect.stringMatching(/^\d+$/));
    expect(apiEnv.OTTIE_URL).toBe(apiEnv.OTTIE_SERVICE_API_URL);
    expect(apiEnv.OTTIE_PORT).toBe(apiEnv.OTTIE_SERVICE_API_PORT);
    expect(apiEnv).not.toHaveProperty("PORT");

    expect(webEnv.OTTIE_SERVICE_API_URL).toBe("http://api.feature-peer-env.repo.localhost:6767");
    expect(webEnv.OTTIE_SERVICE_WEB_URL).toBe("http://web.feature-peer-env.repo.localhost:6767");
    expect(webEnv.OTTIE_SERVICE_API_PORT).toBe(apiEnv.OTTIE_SERVICE_API_PORT);
    expect(webEnv.OTTIE_SERVICE_WEB_PORT).toBe(apiEnv.OTTIE_SERVICE_WEB_PORT);
    expect(webEnv.OTTIE_URL).toBe(webEnv.OTTIE_SERVICE_WEB_URL);
    expect(webEnv.OTTIE_PORT).toBe(webEnv.OTTIE_SERVICE_WEB_PORT);
    expect(webEnv).not.toHaveProperty("PORT");

    const apiPort = Number(apiEnv.OTTIE_SERVICE_API_PORT);
    const webPort = Number(apiEnv.OTTIE_SERVICE_WEB_PORT);
