...
describe("bootstrap provider availability", () => {
  ...
  test("loads a persisted Codex record without spawning a missing Codex binary", async () => {
    const { createOttieDaemon } = await import("./bootstrap.js");
    const root = await mkdtemp(path.join(os.tmpdir(), "ottie-bootstrap-provider-"));
    tempRoots.push(root);
    const binDir = await mkdtemp(path.join(os.tmpdir(), "ottie-bootstrap-provider-bin-"));
    tempRoots.push(binDir);
    process.env.PATH = binDir;
    if (process.platform === "win32") {
      process.env.PATHEXT = ".CMD";
    }
    const ottieHome = path.join(root, ".ottie");
    const staticDir = path.join(root, "static");
    const agentStoragePath = path.join(ottieHome, "agents");
    ...
    const daemon = await createOttieDaemon(config, pino({ level: "silent" }));
    try {
