...
it("registerRoute and findRoute with exact match", () => {
  ...
  store.registerRoute({
    ...
    port: 3000,
    workspaceId: "/repo/.ottie/worktrees/feature-a",
    projectSlug: "repo",
    scriptName: "editor",
  });
...
it("listRoutes returns enriched entries", () => {
  ...
  store.registerRoute({
    ...
    port: 4000,
    workspaceId: "/repo/.ottie/worktrees/feature-b",
    projectSlug: "repo",
    scriptName: "docs",
  });
...
it("listRoutesForWorkspace returns only routes for that workspace", () => {
  ...
  expect(store.listRoutesForWorkspace("/repo/.ottie/worktrees/feature-a")).toEqual([
    {
      hostname: "a.localhost",
      port: 3000,
      workspaceId: "/repo/.ottie/worktrees/feature-a",
