...
function createWorkspaceRepo(options?: {
  branchName?: string;
  ottieConfig?: Record<string, unknown>;
...
describe("script-route-branch-handler", () => {
  it("updates routes on branch rename by removing old hostnames and registering new ones", () => {
    ...
    registerRoute(routeStore, {
      hostname: "api.feature-auth.ottie.localhost",
      port: 3001,
      scriptName: "api",
    });
    ...
    expect(routeStore.findRoute("api.feature-auth.ottie.localhost")).toBeNull();
    expect(routeStore.findRoute("api.feature-billing.ottie.localhost")).toEqual({
      hostname: "api.feature-billing.ottie.localhost",
      port: 3001,
    });
  });
  ...
  it("updates all services for a workspace when multiple routes are registered", () => {
    ...
    registerRoute(routeStore, {
      hostname: "web.feature-auth.ottie.localhost",
      port: 3002,
      scriptName: "web",
    });
    registerRoute(routeStore, {
    ...
    expect(routeStore.listRoutesForWorkspace("workspace-a")).toEqual([
      ...
      {
        hostname: "web.feature-billing.ottie.localhost",
        port: 3002,
        workspaceId: "workspace-a",
        projectSlug: "ottie",
        scriptName: "web",
