...
function getServerIdPath(ottieHome: string): string {
  return path.join(ottieHome, SERVER_ID_FILENAME);
}
...
/**
 * Stable daemon identifier scoped to a given $OTTIE_HOME.
 *
 * - Persisted to `$OTTIE_HOME/server-id`
 * - Can be overridden via `OTTIE_SERVER_ID` (useful for tests)
 */
export function getOrCreateServerId(
  ...
  const serverIdPath = getServerIdPath(ottieHome);

  const envOverride =
    typeof env.OTTIE_SERVER_ID === "string" && env.OTTIE_SERVER_ID.trim().length > 0
      ? env.OTTIE_SERVER_ID.trim()
      : null;
  ...
  if (envOverride) {
    ...
    if (!existsSync(serverIdPath)) {
      try {
        ...
        log?.info({ serverId: envOverride }, "Persisted OTTIE_SERVER_ID override");
      } catch (error) {
        log?.warn({ error }, "Failed to persist OTTIE_SERVER_ID override");
      }
    }
    return envOverride;
  }
