...
describe("LoopService", () => {
  ...
  beforeEach(() => {
    ...
    ottieHome = path.join(tmpDir, "ottie-home");
  ...
  test("runs fresh worker agents until verify-check passes", async () => {
    ...
    const service = new LoopService({ ottieHome, agentManager: manager, logger });
    ...
    expect(readFileSync(path.join(ottieHome, "loops", "loops.json"), "utf8")).toContain(loop.id);
  });
