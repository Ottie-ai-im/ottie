...
export type OttieOpenAIConfig = OpenAiSpeechProviderConfig;
export type OttieLocalSpeechConfig = LocalSpeechProviderConfig;

export interface OttieSpeechConfig {
  providers: RequestedSpeechProviders;
  local?: OttieLocalSpeechConfig;
}
...
export interface OttieDaemonConfig {
  listen: string;
  ottieHome: string;
...
export async function createOttieDaemon(
  config: OttieDaemonConfig,
  rootLogger: Logger,
): Promise<OttieDaemon> {
  ...
  const allowedOrigins = new Set([
    ...
    // Packaged desktop renderers use the custom ottie:// protocol scheme.
    "ottie://app",
  ...
  const projectRegistry = new FileBackedProjectRegistry(
    path.join(config.ottieHome, "projects", "projects.json"),
    logger,
  );
  workspaceRegistry = new FileBackedWorkspaceRegistry(
    path.join(config.ottieHome, "projects", "workspaces.json"),
    logger,
  );
  ...
  if (mcpEnabled) {
    ...
    const createAgentMcpTransport = async (callerAgentId?: string) => {
      const agentMcpServer = await createAgentMcpServer({
        ...
        createOttieWorktree: async (input, serviceOptions) => {
