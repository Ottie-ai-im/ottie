...
export function buildWorkspaceServiceEnv(
  ...
  const env: Record<string, string> = {
    ...
    OTTIE_PORT: String(selfPeer.port),
  };

  if (options.daemonPort !== null && options.daemonPort !== undefined) {
    env.OTTIE_URL = buildServiceProxyUrl({
  ...
  for (const peer of options.peers) {
    ...
    env[`OTTIE_SERVICE_${envName}_PORT`] = String(peer.port);

    if (options.daemonPort !== null && options.daemonPort !== undefined) {
      env[`OTTIE_SERVICE_${envName}_URL`] = buildServiceProxyUrl({
        projectSlug: options.projectSlug,
        branchName: options.branchName,
