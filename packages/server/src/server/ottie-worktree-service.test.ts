...
import { createOttieWorktree, type CreateOttieWorktreeDeps } from "./ottie-worktree-service.js";
...
test("creates a worktree and registers it in the source workspace project without git snapshot lookup", async () => {
  ...
  const result = await createOttieWorktree(
    {
      cwd: repoDir,
...
test("reuses an existing worktree and still upserts the workspace", async () => {
  ...
  const first = await createOttieWorktree(
    {
      cwd: repoDir,
  ...
  const second = await createOttieWorktree(
    {
      cwd: repoDir,
      worktreeSlug: "reuse-me",
      runSetup: false,
...
test("does not mutate registries or broadcast when core worktree creation fails", async () => {
  const tempDir = mkdtempSync(path.join(tmpdir(), "ottie-worktree-service-"));
  cleanupPaths.push(tempDir);
  const deps = createDeps();
...
test("keeps direct core worktree creation calls behind the service boundary", () => {
  ...
  expect(matches).toEqual(["ottie-worktree-service.test.ts", "ottie-worktree-service.ts"]);
});

interface TestDeps extends CreateOttieWorktreeDeps {
...
function createWorkspaceGitSnapshot(cwd: string): WorkspaceGitRuntimeSnapshot {
  ...
  return {
    ...
    git: {
      ...
      currentBranch,
      remoteUrl: null,
      isOttieOwnedWorktree: repoRoot !== mainRepoRoot,
      isDirty: false,
      baseRef: "main",
      aheadBehind: null,
      aheadOfOrigin: null,
