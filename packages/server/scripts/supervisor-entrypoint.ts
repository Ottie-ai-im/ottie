...
function resolvePackagedNodeEntrypointRunnerPath(currentScriptPath: string): string | null {
  const packageMarker = `${path.sep}node_modules${path.sep}@ottie${path.sep}server${path.sep}`;
...
async function main(): Promise<void> {
  ...
  const workerEnv: NodeJS.ProcessEnv = { ...process.env, OTTIE_SUPERVISED: "1" };
  ...
  const ottieHome = resolveOttieHome(workerEnv);
  ...
  runSupervisor({
    ...
    onWorkerReady: async ({ listen }) => {
      await updatePidLock(ottieHome, { listen }, { ownerPid: process.pid });
    },
