...
function parseLifecycleMessage(msg: unknown): WorkerLifecycleMessage | null {
  ...
  if (type === "ottie:shutdown") {
    return { type: "ottie:shutdown" };
  }
  if (type === "ottie:ready") {
    ...
    return { type: "ottie:ready", listen };
  }
  if (type === "ottie:restart") {
...
export function runSupervisor(options: SupervisorOptions): void {
  ...
  const spawnWorker = () => {
    ...
    child.on("message", (msg: unknown) => {
      ...
      if (lifecycleMessage.type === "ottie:ready") {
      ...
      if (lifecycleMessage.type === "ottie:shutdown") {
