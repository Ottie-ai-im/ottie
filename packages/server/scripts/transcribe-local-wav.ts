...
function usage(): string {
  return [
    ...
    "",
    "Env fallbacks:",
    "  OTTIE_LOCAL_MODELS_DIR, OTTIE_LOCAL_STT_MODEL",
  ].join("\n");
}
...
function parseArgs(argv: string[]): CliOptions {
  ...
  const defaultModelsDir =
    process.env.OTTIE_LOCAL_MODELS_DIR ?? path.join(ottieHome, "models", "local-speech");

  const positional: string[] = [];
  let outPath: string | undefined;
  let model = LocalSttModelIdSchema.parse(
    process.env.OTTIE_LOCAL_STT_MODEL ?? DEFAULT_LOCAL_STT_MODEL,
  );
