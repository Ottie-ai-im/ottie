...
function parseArgs(argv: string[]): { modelsDir: string; modelIds: LocalSpeechModelId[] } {
  const home = resolveOttieHome();
  let modelsDir = process.env.OTTIE_LOCAL_MODELS_DIR || `${home}/models/local-speech`;
  ...
  if (modelIds.length === 0) {
    const stt = (process.env.OTTIE_LOCAL_STT_MODEL ||
      DEFAULT_LOCAL_STT_MODEL) as LocalSpeechModelId;
    const tts = (process.env.OTTIE_LOCAL_TTS_MODEL ||
