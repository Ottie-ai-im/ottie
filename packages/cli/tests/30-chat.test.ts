...
try {
...

  {
    ...
    const posted = await ctx.ottie(["chat", "post", "coord-room", "first message for @agent-1"], {
      env: { OTTIE_AGENT_ID: "00000000-0000-4000-8000-000000000111" },
    });
    ...
    const read = await ctx.ottie(["chat", "read", "coord-room", "--limit", "10"]);
    ...
    const readJson = await ctx.ottie(["chat", "read", "coord-room", "--limit", "10", "--json"]);
    ...
    const waitPromise = ctx.ottie(["chat", "wait", "coord-room", "--timeout", "5s"]);
    await new Promise((resolve) => setTimeout(resolve, 250));
    const secondPost = await ctx.ottie(["chat", "post", "coord-room", "second message"]);
...

  {
    ...
    const duplicate = await ctx.ottie(["chat", "create", "coord-room"]);
...

  {
    ...
    const deleted = await ctx.ottie(["chat", "delete", "coord-room"]);
  ...
  await ctx.stop();
  await rm(ctx.ottieHome, { recursive: true, force: true });
