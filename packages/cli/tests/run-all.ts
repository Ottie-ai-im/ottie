...
/**
 * Test runner for Ottie CLI E2E tests
 *
...
async function writeJsonSummary({
  ...
  await writeFile(
    ...
    JSON.stringify(
      {
        suite: "cli-local",
        command: "npm run test:local --workspace=@ottie/cli",
        counts: {
          passed,
          failed,
          skipped: 0,
...
console.log("🧪 Ottie CLI E2E Test Runner\n");
console.log("=".repeat(50));
...
await runCommand("Building relay", "npm run build --workspace=@ottie/relay");
await runCommand("Building server", "npm run build --workspace=@ottie/server");
await runCommand("Building CLI", "npm run build --workspace=@ottie/cli");
...
async function runSingleTest(testFile: string): Promise<TestOutcome> {
  ...
  try {
    const result =
      await $`OTTIE_LOCAL_SPEECH_AUTO_DOWNLOAD=${testEnvDefaults.OTTIE_LOCAL_SPEECH_AUTO_DOWNLOAD} OTTIE_DICTATION_ENABLED=${testEnvDefaults.OTTIE_DICTATION_ENABLED} OTTIE_VOICE_MODE_ENABLED=${testEnvDefaults.OTTIE_VOICE_MODE_ENABLED} npx tsx ${testPath}`.nothrow();
    if (result.exitCode === 0) {
      console.log(`\n✅ ${testName} PASSED`);
