...
const promptFilePath = join(ottieHome, "send-prompt.txt");
...
try {
  // Test 1: send --help shows options
  {
    ...
    const result = await $`npx ottie send --help`.nothrow();
  ...
  // Test 2: send requires id argument
  {
    ...
    const result =
      await $`OTTIE_HOST=localhost:${port} OTTIE_HOME=${ottieHome} npx ottie send`.nothrow();
  ...
  // Test 3: send requires prompt argument
  {
    ...
    const result =
      await $`OTTIE_HOST=localhost:${port} OTTIE_HOME=${ottieHome} npx ottie send abc123`.nothrow();
  ...
  // Test 4: send handles daemon not running
  {
    ...
    const result =
      await $`OTTIE_HOST=localhost:${port} OTTIE_HOME=${ottieHome} npx ottie send abc123 "test prompt"`.nothrow();
  ...
  // Test 5: send --no-wait flag is accepted
  {
    ...
    const result =
      await $`OTTIE_HOST=localhost:${port} OTTIE_HOME=${ottieHome} npx ottie send --no-wait abc123 "test prompt"`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 5b: send --prompt flag is accepted
  {
    ...
    const result =
      await $`OTTIE_HOST=localhost:${port} OTTIE_HOME=${ottieHome} npx ottie send --prompt "test prompt" abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 5c: send --prompt-file flag is accepted
  {
    ...
    const result =
      await $`OTTIE_HOST=localhost:${port} OTTIE_HOME=${ottieHome} npx ottie send --prompt-file ${promptFilePath} abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 6: send --host flag is accepted
  {
    ...
    const result =
      await $`OTTIE_HOME=${ottieHome} npx ottie send --host localhost:${port} abc123 "test prompt"`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 7: -q (quiet) flag is accepted with send
  {
    ...
    const result =
      await $`OTTIE_HOST=localhost:${port} OTTIE_HOME=${ottieHome} npx ottie -q send --no-wait abc123 "test prompt"`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 8: Combined flags work together
  {
    ...
    const result =
      await $`OTTIE_HOST=localhost:${port} OTTIE_HOME=${ottieHome} npx ottie -q send --no-wait abc123 "Run the linter"`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 8b: conflicting prompt sources are rejected
  {
    ...
    const result =
      await $`OTTIE_HOST=localhost:${port} OTTIE_HOME=${ottieHome} npx ottie send abc123 "positional prompt" --prompt "flag prompt"`.nothrow();
  ...
  // Test 9: ottie --help shows send command
  {
    console.log("Test 9: ottie --help shows send command");
    ...
    console.log("✓ ottie --help shows send command\n");
  }
