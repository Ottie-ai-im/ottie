 ...
 * 2. Create agent with `ottie run "say hello" --provider claude`
 * 3. List agents with `ottie ls`
 * 4. Wait for agent with `ottie wait <id>`
 * 5. Inspect agent with `ottie inspect <id>`
 * 6. Stop agent with `ottie stop <id>` and verify it remains inspectable
 * 7. Delete agent with `ottie delete <id>`
...
async function setup(): Promise<void> {
  ...
  try {
    ...
    console.log(`OTTIE_HOME: ${ctx.ottieHome}`);
...
async function test_agent_ls_empty(): Promise<void> {
  ...
  const result = await ctx.ottie(["ls", "--json"]);
...
async function test_agent_wait(agentId: string): Promise<void> {
  ...
  const result = await ctx.ottie(["wait", "--timeout", "120s", agentId], { timeout: 130000 });
...
async function test_agent_inspect(agentId: string): Promise<void> {
  ...
  const result = await ctx.ottie(["inspect", agentId]);
...
async function test_agent_logs(agentId: string): Promise<void> {
  ...
  const result = await ctx.ottie(["logs", "--tail", "20", agentId]);
...
async function test_agent_stop(agentId: string): Promise<void> {
  ...
  const result = await ctx.ottie(["stop", agentId]);
  ...
  const inspectResult = await ctx.ottie(["inspect", agentId]);
...
async function test_agent_delete(agentId: string): Promise<void> {
  ...
  const result = await ctx.ottie(["delete", agentId]);
