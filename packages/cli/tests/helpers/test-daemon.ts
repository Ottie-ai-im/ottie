 ...
 * Test Daemon Helper
 *
 * Provides utilities for launching real Ottie daemons in E2E tests.
 * Each test gets an isolated daemon on an available local port with its own OTTIE_HOME.
 *
...
const TEST_OUTPUT_CAPTURE_LIMIT = Number.parseInt(
  process.env.OTTIE_TEST_OUTPUT_CAPTURE_BYTES ?? `${DEFAULT_OUTPUT_CAPTURE_LIMIT}`,
  10,
);
...
export async function createTempDirs(): Promise<{ ottieHome: string; workDir: string }> {
  const ottieHome = await mkdtemp(join(tmpdir(), "ottie-e2e-home-"));
  const workDir = await mkdtemp(join(tmpdir(), "ottie-e2e-work-"));

  // Create the agents directory that the daemon expects
  const agentsDir = join(ottieHome, "agents");
...
/**
 * Wait for daemon to be ready by running `ottie agent ls`
...
async function probeDaemonReady(port: number): Promise<boolean> {
  try {
    const { exitCode } = await runOttieCli(
      {
        port,
 ...
 * with isolated OTTIE_HOME and OTTIE_LISTEN environment variables.
 */
export async function startTestDaemon(options?: {
  ...
  const { ottieHome, workDir } =
    options?.ottieHome && options?.workDir
      ? { ottieHome: options.ottieHome, workDir: options.workDir }
  ...
  const daemonProcess = spawn("npx", ["tsx", cliSrcPath, "daemon", "start", "--foreground"], {
    env: {
      ...
      OTTIE_LISTEN: `${TEST_DAEMON_HOST}:${port}`,
  ...
  return new Promise((resolve, reject) => {
    const proc = spawn("npx", ["tsx", cliSrcPath, ...args], {
      env: {
        ...process.env,
        ...TEST_DAEMON_ENV_DEFAULTS,
        OTTIE_HOST: `${TEST_DAEMON_HOST}:${ctx.port}`,
        OTTIE_HOME: ctx.ottieHome,
        ...options?.env,
      },
      cwd,
      stdio: ["ignore", "pipe", "pipe"],
    ...
    const timeoutId = setTimeout(() => {
      ...
      reject(new Error(`CLI command timed out after ${timeout}ms: ottie ${args.join(" ")}`));
    }, timeout);
...
export async function createE2ETestContext(options?: { timeout?: number }): Promise<
  TestDaemonContext & {
    /** Run a ottie CLI command against this daemon */
    ottie: (
      args: string[],
