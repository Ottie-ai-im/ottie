...
const ottieHome = await mkdtemp(join(tmpdir(), "ottie-restart-supervisor-"));
...
try {
  ...
  const statusBeforeRestart = await readDaemonStatus(ottieHome);
  ...
  const statusAfterRestart = await readDaemonStatus(ottieHome);
  assert.strictEqual(
