...
async function runProviderModelsJson(provider: string): Promise<ProviderModel[]> {
  ...
  async function attemptRun(attempt: number): Promise<ProviderModel[]> {
    const result = await ctx.ottie(["provider", "models", provider, "--json"]);
...
try {
  // Test 1: provider --help shows subcommands
  {
    ...
    const result = await ctx.ottie(["provider", "--help"]);
  ...
  // Test 2: provider ls lists all providers
  {
    ...
    const result = await ctx.ottie(["provider", "ls"]);
  ...
  // Test 3: provider ls --json outputs valid JSON
  {
    ...
    const result = await ctx.ottie(["provider", "ls", "--json"]);
  ...
  // Test 4: provider ls --quiet outputs provider names only
  {
    ...
    const result = await ctx.ottie(["provider", "ls", "--quiet"]);
    assert.strictEqual(result.exitCode, 0, "should exit 0");
  ...
  // Test 8: provider models unknown fails with error
  {
    ...
    const result = await ctx.ottie(["provider", "models", "unknown"]);
  ...
  // Test 10: provider models --quiet outputs model IDs only
  {
    ...
    const result = await ctx.ottie(["provider", "models", "claude", "--quiet"]);
    assert.strictEqual(result.exitCode, 0, "should exit 0");
    const lines = result.stdout.trim().split("\n").filter(Boolean);
    assert.strictEqual(
      lines.length,
