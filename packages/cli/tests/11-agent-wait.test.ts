...
try {
  // Test 1: wait --help shows options
  {
    ...
    const result = await $`npx ottie wait --help`.nothrow();
  ...
  // Test 2: wait requires id argument
  {
    ...
    const result =
      await $`OTTIE_HOST=localhost:${port} OTTIE_HOME=${ottieHome} npx ottie wait`.nothrow();
  ...
  // Test 3: wait handles daemon not running
  {
    ...
    const result =
      await $`OTTIE_HOST=localhost:${port} OTTIE_HOME=${ottieHome} npx ottie wait abc123`.nothrow();
  ...
  // Test 4: wait --timeout flag is accepted
  {
    ...
    const result =
      await $`OTTIE_HOME=${ottieHome} npx ottie wait --timeout 30 --host localhost:${port} abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 5: wait --host flag is accepted
  {
    ...
    const result =
      await $`OTTIE_HOME=${ottieHome} npx ottie wait --host localhost:${port} abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 6: -q (quiet) flag is accepted with wait
  {
    ...
    const result =
      await $`OTTIE_HOST=localhost:${port} OTTIE_HOME=${ottieHome} npx ottie -q wait abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 7: --json flag is accepted with wait
  {
    ...
    const result =
      await $`OTTIE_HOST=localhost:${port} OTTIE_HOME=${ottieHome} npx ottie wait abc123 --json`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 8: --format yaml flag is accepted with wait
  {
    ...
    const result =
      await $`OTTIE_HOST=localhost:${port} OTTIE_HOME=${ottieHome} npx ottie --format yaml wait abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 9: ottie --help shows wait command
  {
    console.log("Test 9: ottie --help shows wait command");
    ...
    console.log("ottie --help shows wait command\n");
  }
