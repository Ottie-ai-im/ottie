...
const ottieHome = await mkdtemp(join(tmpdir(), "ottie-onboard-home-"));
const port = await getAvailablePort();

try {
  console.log("Test 1: `ottie` runs blocking onboarding and prints pairing info");
  const onboard =
    await $`OTTIE_HOME=${ottieHome} OTTIE_LISTEN=127.0.0.1:${port} OTTIE_PAIRING_QR=0 npx ottie`.nothrow();
  ...
  assert(onboard.stdout.includes("ottie --help"), "onboard output should include --help shortcut");
  assert(onboard.stdout.includes("ottie ls"), "onboard output should include ls shortcut");
  assert(
    onboard.stdout.includes('ottie run "your prompt"'),
    "onboard output should include run shortcut",
  );
  assert(onboard.stdout.includes("ottie status"), "onboard output should include status shortcut");
  assert(
    onboard.stdout.includes(join(ottieHome, "daemon.log")),
    "onboard output should include daemon log path",
  );

  const status =
    await $`OTTIE_HOME=${ottieHome} npx ottie daemon status --home ${ottieHome}`.nothrow();
  ...
  const configRaw = await readFile(join(ottieHome, "config.json"), "utf-8");
  ...
  const daemonLog = await readFile(join(ottieHome, "daemon.log"), "utf-8");
  assert(
  ...
  await $`OTTIE_HOME=${ottieHome} npx ottie daemon stop --home ${ottieHome} --force`.nothrow();
