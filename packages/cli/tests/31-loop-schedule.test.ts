...
try {
  {
    ...
    const listed = await ctx.ottie(["schedule", "ls", "--json"]);
    ...
    const inspected = await ctx.ottie(["schedule", "inspect", createdJson.id, "--json"]);
...

  {
    ...
    const listed = await ctx.ottie(["loop", "ls", "--json"]);
    assert.strictEqual(listed.exitCode, 0, listed.stderr);
    const listedJson = JSON.parse(listed.stdout);
    assert(Array.isArray(listedJson), listed.stdout);
    assert(
    ...
    async function pollStatus(attempt: number): Promise<string> {
      ...
      const inspect = await ctx.ottie(["loop", "inspect", runJson.id, "--json"]);
    ...
    const logs = await ctx.ottie(["loop", "logs", runJson.id], { timeout: 15000 });
    assert.strictEqual(logs.exitCode, 0, logs.stderr);
    assert(logs.stdout.includes("verify-check"), logs.stdout);

    const stopped = await ctx.ottie(["loop", "stop", runJson.id, "--json"]);
