...
async function readPidLockPid(ottieHome: string): Promise<number | null> {
...
const ottieHome = await mkdtemp(join(tmpdir(), "ottie-restart-unsupervised-"));
...
try {
  ...
  const lockPid = await readPidLockPid(ottieHome);
  ...
  await waitFor(
    async () => (await readPidLockPid(ottieHome)) === null,
    15000,
