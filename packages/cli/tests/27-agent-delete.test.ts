...
const ottieHome = await mkdtemp(join(tmpdir(), "ottie-delete-test-home-"));
...
async function runDelete(args: string[]) {
  return $`OTTIE_HOST=localhost:${port} OTTIE_HOME=${ottieHome} npm --prefix ${repoRoot} run cli -- delete ${args}`.nothrow();
}
...
try {
  ...
  {
    console.log("Test 7: ottie --help shows delete command");
    ...
    console.log("✓ ottie --help shows delete command\n");
  }

  {
    console.log("Test 8: -q (quiet) flag is accepted with delete");
    const result =
      await $`OTTIE_HOST=localhost:${port} OTTIE_HOME=${ottieHome} npm --prefix ${repoRoot} run cli -- -q delete abc123`.nothrow();
    const output = result.stdout + result.stderr;
