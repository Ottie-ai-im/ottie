 ...
 *
 * Tests daemon commands with an isolated OTTIE_HOME.
 *
 * Tests:
...
function daemonCommand(args: string[]) {
  return runLocalOttie(["daemon", ...args], { OTTIE_HOME: ottieHome });
}
...
try {
  // Test 1: daemon --help shows subcommands
  {
    ...
    const result = await runLocalOttie(["daemon", "--help"]);
  ...
  // Test 5: daemon status --json outputs valid JSON
  {
    ...
    assert.strictEqual(status.home, ottieHome, "json status should reflect the isolated home");
    assert.strictEqual(
      status.hostname,
      null,
