...
/**
 * Regression: `ottie daemon stop` must stop supervised dev daemons
...
async function readPidLockState(ottieHome: string): Promise<PidLockState> {
  const pidPath = join(ottieHome, "ottie.pid");
...
async function readDaemonStatus(ottieHome: string): Promise<DaemonStatus> {
  const result =
    await $`OTTIE_HOME=${ottieHome} OTTIE_LOCAL_SPEECH_AUTO_DOWNLOAD=${testEnv.OTTIE_LOCAL_SPEECH_AUTO_DOWNLOAD} OTTIE_DICTATION_ENABLED=${testEnv.OTTIE_DICTATION_ENABLED} OTTIE_VOICE_MODE_ENABLED=${testEnv.OTTIE_VOICE_MODE_ENABLED} npx ottie daemon status --home ${ottieHome} --json`.nothrow();
...
const ottieHome = await mkdtemp(join(tmpdir(), "ottie-stop-supervisor-"));
...
try {
  console.log("Test 1: start supervisor-entrypoint in dev mode with isolated OTTIE_HOME");
  ...
  supervisorProcess = spawn(
    ...
    ["--import", "tsx", "../server/scripts/supervisor-entrypoint.ts", "--dev"],
    {
      ...
      env: {
        ...
        OTTIE_LISTEN: `127.0.0.1:${port}`,
        OTTIE_RELAY_ENABLED: "false",
        CI: "true",
      },
  ...
  await waitFor(
    async () => {
      const status = await readDaemonStatus(ottieHome);
      return (
  ...
  const statusBeforeStop = await readDaemonStatus(ottieHome);
  ...
  const pidLockBeforeStop = await readPidLockState(ottieHome);
  ...
  console.log("Test 2: `ottie daemon stop` should stop without respawn");
  const stopResult =
    await $`OTTIE_HOME=${ottieHome} OTTIE_LOCAL_SPEECH_AUTO_DOWNLOAD=${testEnv.OTTIE_LOCAL_SPEECH_AUTO_DOWNLOAD} OTTIE_DICTATION_ENABLED=${testEnv.OTTIE_DICTATION_ENABLED} OTTIE_VOICE_MODE_ENABLED=${testEnv.OTTIE_VOICE_MODE_ENABLED} npx ottie daemon stop --home ${ottieHome} --json`.nothrow();
  ...
  const pidAfterStop = await readPidLockState(ottieHome);
  ...
  const statusAfterStop = await readDaemonStatus(ottieHome);
  assert.strictEqual(
  ...
  await $`OTTIE_HOME=${ottieHome} OTTIE_LOCAL_SPEECH_AUTO_DOWNLOAD=${testEnv.OTTIE_LOCAL_SPEECH_AUTO_DOWNLOAD} OTTIE_DICTATION_ENABLED=${testEnv.OTTIE_DICTATION_ENABLED} OTTIE_VOICE_MODE_ENABLED=${testEnv.OTTIE_VOICE_MODE_ENABLED} npx ottie daemon stop --home ${ottieHome} --force`.nothrow();
