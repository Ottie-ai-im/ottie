...
try {
  // Test 1: inspect --help shows options
  {
    ...
    const result = await $`npx ottie inspect --help`.nothrow();
  ...
  // Test 2: inspect requires id argument
  {
    ...
    const result =
      await $`OTTIE_HOST=localhost:${port} OTTIE_HOME=${ottieHome} npx ottie inspect`.nothrow();
  ...
  // Test 3: inspect handles daemon not running
  {
    ...
    const result =
      await $`OTTIE_HOST=localhost:${port} OTTIE_HOME=${ottieHome} npx ottie inspect abc123`.nothrow();
  ...
  // Test 4: inspect --host flag is accepted
  {
    ...
    const result =
      await $`OTTIE_HOME=${ottieHome} npx ottie inspect --host localhost:${port} abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 5: -q (quiet) flag is accepted with inspect
  {
    ...
    const result =
      await $`OTTIE_HOST=localhost:${port} OTTIE_HOME=${ottieHome} npx ottie -q inspect abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 6: --json flag is accepted with inspect
  {
    ...
    const result =
      await $`OTTIE_HOST=localhost:${port} OTTIE_HOME=${ottieHome} npx ottie inspect abc123 --json`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 7: --format yaml flag is accepted with inspect
  {
    ...
    const result =
      await $`OTTIE_HOST=localhost:${port} OTTIE_HOME=${ottieHome} npx ottie --format yaml inspect abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 8: ottie --help shows inspect command
  {
    console.log("Test 8: ottie --help shows inspect command");
    ...
    console.log("ottie --help shows inspect command\n");
  }
