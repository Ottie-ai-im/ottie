...
import type { DaemonClient, AgentStreamMessage, AgentTimelineItem } from "@ottie/server";
import { curateAgentActivity } from "@ottie/server";
...
export async function runLogsCommand(
  ...
  if (!id) {
    ...
    console.error("Usage: ottie agent logs <id>");
    process.exit(1);
  }
  ...
  try {
    ...
    console.error("Start the daemon with: ottie daemon start");
    process.exit(1);
  }
  ...
  try {
    ...
    if (!fetchResult) {
      ...
      console.error("Use `ottie ls` to list available agents");
      await client.close();
      process.exit(1);
    }
