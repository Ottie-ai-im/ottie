...
import type { AgentSnapshotPayload } from "@ottie/server";
...
function toInspectData(snapshot: AgentSnapshotPayload): AgentInspect {
  return {
    ...
    Worktree: snapshot.labels?.["ottie.worktree"] ?? null,
    ParentAgentId: snapshot.labels?.["ottie.parent-agent-id"] ?? null,
  };
}
...
export async function runInspectCommand(
  ...
  if (!agentIdArg || agentIdArg.trim().length === 0) {
    const error: CommandError = {
      ...
      details: "Usage: ottie agent inspect <id>",
    };
    throw error;
  }
