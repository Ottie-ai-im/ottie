...
function findDesktopApp(): string | null {
  if (process.platform === "darwin") {
    const candidates = [
      "/Applications/Ottie.app",
      path.join(homedir(), "Applications", "Ottie.app"),
    ];
  ...
  if (process.platform === "linux") {
    const candidates = [
      "/usr/bin/Ottie",
      "/opt/Ottie/Ottie",
      path.join(homedir(), "Applications", "Ottie.AppImage"),
    ];
  ...
  if (process.platform === "win32") {
    ...
    const candidate = path.join(localAppData, "Programs", "Ottie", "Ottie.exe");
...
export async function openDesktopWithProject(projectPath: string): Promise<void> {
  try {
    if (process.env.OTTIE_DESKTOP_CLI === "1") {
      throw new Error(
    ...
    if (!desktopApp) {
      throw new Error(
        "Ottie desktop app not found. Install it from https://github.com/ottie/ottie/releases",
      );
    }
