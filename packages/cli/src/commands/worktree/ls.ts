...
export function resolveOttieHomePath(): string {
  return process.env.OTTIE_HOME ?? join(homedir(), ".ottie");
}

export function resolveOttieWorktreesDir(): string {
  return join(resolveOttieHomePath(), "worktrees");
}

function isAgentInManagedWorktree(agentCwd: string): boolean {
  const worktreesDir = resolveOttieWorktreesDir();
...
export async function runLsCommand(
  ...
  try {
    ...
    const error: CommandError = {
      ...
      details: "Start the daemon with: ottie daemon start",
    };
    throw error;
  }
  ...
  try {
    ...
    const response = await client.getOttieWorktreeList({});
