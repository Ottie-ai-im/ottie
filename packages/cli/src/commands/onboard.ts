...
function savePersistedConfig(ottieHome: string, config: OnboardPersistedConfig): void {
  const configPath = path.join(ottieHome, "config.json");
...
function printNextSteps(pairingUrl: string | null, ottieHome: string, richUi: boolean): void {
  const daemonLogPath = path.join(ottieHome, "daemon.log");
...
async function resolveAndPersistVoice(
  ...
  let persisted = loadPersistedConfig(ottieHome) as OnboardPersistedConfig;
  ...
  savePersistedConfig(ottieHome, persisted);
  return voiceEnabled;
}
...
export async function runOnboard(options: OnboardOptions): Promise<void> {
  ...
  const ottieHome = resolveLocalOttieHome(options.home);
  if (richUi) {
    renderNote(ottieHome, "Ottie home");
  }

  const voiceEnabled = await resolveAndPersistVoice(ottieHome, options);
  const config = loadConfig(ottieHome, { cli: toCliOverrides(options) });
  ...
  if (config.relayEnabled === false) {
    ...
    printNextSteps(null, ottieHome, richUi);
    if (richUi) {
      outro("Ottie daemon is running.");
    }
    return;
  }
  ...
  printNextSteps(pairing.url, ottieHome, richUi);
  if (richUi) {
    outro("Ottie is ready!");
  }
}
