...
import { loadConfig, resolveOttieHome, DaemonClient } from "@ottie/server";
...
function readPidSocketTarget(ottieHome: string): string | null {
  const pidPath = path.join(ottieHome, PID_FILENAME);
...
function resolveConfiguredIpcDaemonHost(env: NodeJS.ProcessEnv, ottieHome: string): string | null {
  const directEnvHost = normalizeDaemonHost(env.OTTIE_LISTEN ?? "");
  if (isIpcDaemonHost(directEnvHost)) {
    return directEnvHost;
  }

  const pidHost = normalizeDaemonHost(readPidSocketTarget(ottieHome) ?? "");
  if (isIpcDaemonHost(pidHost)) {
    return pidHost;
  }

  const config = loadConfig(ottieHome, { env });
  const configuredHost = normalizeDaemonHost(config.listen);
  return isIpcDaemonHost(configuredHost) ? configuredHost : null;
}

function resolveConfiguredTcpDaemonHost(env: NodeJS.ProcessEnv, ottieHome: string): string | null {
  const configuredHost = normalizeDaemonHost(loadConfig(ottieHome, { env }).listen);
...
export function resolveDefaultDaemonHosts(env: NodeJS.ProcessEnv = process.env): string[] {
  ...
  const configuredIpcHost = resolveConfiguredIpcDaemonHost(env, ottieHome);
  if (configuredIpcHost) {
    candidates.push(configuredIpcHost);
  }
  const configuredTcpHost = resolveConfiguredTcpDaemonHost(env, ottieHome);
...
function resolveDaemonHostCandidates(options?: ConnectOptions): string[] {
  const explicitHost = options?.host ?? process.env.OTTIE_HOST;
  if (explicitHost) {
    return [explicitHost];
  }
...
export async function connectToDaemon(options?: ConnectOptions): Promise<DaemonClient> {
  ...
  async function tryNext(index: number, lastError: unknown): Promise<DaemonClient> {
    if (index >= hosts.length) {
      ...
      throw new Error(`Unable to connect to Ottie daemon via ${hosts.join(", ")}`);
    }
