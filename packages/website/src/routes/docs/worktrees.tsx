    ...
    <div className="space-y-10">
      <div>
        ...
        <p className="text-white/60 leading-relaxed">
          ...
          services through a <code className="font-mono">ottie.json</code> file at your repo root.
        </p>
      </div>
      ...
      <section className="space-y-4">
        ...
        <p className="text-white/60 leading-relaxed">
          Worktrees live under <code className="font-mono">$OTTIE_HOME/worktrees/</code>, grouped by
        ...
        <Code>
          <pre className="text-white/80">{`~/.ottie/worktrees/
└── 1vnnm9k3/               # hash of source checkout path
    ...
    └── bold-owl/`}</pre>
        ...
        <ol className="text-white/60 space-y-2 list-decimal list-inside">
          <li>Create a worktree — Ottie runs your setup hooks</li>
      ...
      {/* ottie.json */}
      <section className="space-y-4">
        <h2 className="text-xl font-medium">ottie.json</h2>
        <p className="text-white/60 leading-relaxed">
          Drop a <code className="font-mono">ottie.json</code> in your repo root. Ottie reads it
  ...
  "worktree": {
    "setup": [
      "npm ci",
      "cp \\"$OTTIE_SOURCE_CHECKOUT_PATH/.env\\" .env",
      "npm run db:migrate"
    ],
    "teardown": [
      "npm run db:drop || true"
        ...
        <p className="text-white/60 leading-relaxed">
          ...
          <code className="font-mono">$OTTIE_SOURCE_CHECKOUT_PATH</code> to reach files in the
      ...
      <section className="space-y-4">
        ...
        <p className="text-white/60 leading-relaxed">
          ...
          worktree on demand. Mark one as a <em>service</em> and Ottie supervises it as a
  ...
  "scripts": {
    "web": {
      "type": "service",
      "command": "npm run dev -- --port $OTTIE_PORT",
      "port": 3000
    },
    "api": {
      "type": "service",
      "command": "npm run api -- --port $OTTIE_PORT"
    }
  }
}`}</pre>
        </Code>
        <p className="text-white/60 leading-relaxed">
          Omit <code className="font-mono">port</code> to let Ottie auto-assign one. Bind your
          process to <code className="font-mono">$OTTIE_PORT</code> rather than hard-coding — each
...
http://<script>.<project>.localhost:<daemon-port>`}</pre>
        ...
        <Code>
          <pre className="text-white/80">{`OTTIE_PORT=3000                         # this service's port
OTTIE_URL=http://web.my-app.localhost:6767  # this service's proxy URL
OTTIE_SERVICE_API_PORT=51732
OTTIE_SERVICE_API_URL=http://api.my-app.localhost:6767
OTTIE_SERVICE_WEB_PORT=3000
OTTIE_SERVICE_WEB_URL=http://web.my-app.localhost:6767`}</pre>
        </Code>
        <p className="text-white/60 leading-relaxed">
          Script names are upper-cased and non-alphanumerics become{" "}
          <code className="font-mono">_</code>. Point your frontend at{" "}
          <code className="font-mono">$OTTIE_SERVICE_API_URL</code> instead of hard-coding a port.
        </p>
      </section>
    ...
    "terminals": [
      ...
      <section className="space-y-4">
        ...
        <ul className="text-white/60 space-y-2 list-disc list-inside">
          <li>
            <code className="font-mono">$OTTIE_SOURCE_CHECKOUT_PATH</code> — the original repo root
          </li>
          <li>
            <code className="font-mono">$OTTIE_WORKTREE_PATH</code> — the worktree directory
          </li>
          <li>
            <code className="font-mono">$OTTIE_BRANCH_NAME</code> — the worktree&apos;s branch
          </li>
          <li>
            <code className="font-mono">$OTTIE_WORKTREE_PORT</code> — legacy per-worktree port
            (prefer <code className="font-mono">$OTTIE_PORT</code> inside services)
          </li>
        </ul>
        <p className="text-white/60 leading-relaxed">Services additionally get:</p>
        <ul className="text-white/60 space-y-2 list-disc list-inside">
          <li>
            <code className="font-mono">$OTTIE_PORT</code> — this service&apos;s assigned port
          </li>
          <li>
            <code className="font-mono">$OTTIE_URL</code> — this service&apos;s proxy URL
          </li>
          <li>
            <code className="font-mono">$OTTIE_SERVICE_&lt;NAME&gt;_PORT</code> /{" "}
      ...
      <section className="space-y-4">
        ...
        <Code>
          <pre className="text-white/80">{`ottie run --worktree feature-auth --base main "implement auth"
ottie worktree ls
ottie worktree archive feature-auth`}</pre>
        </Code>
      </section>
    </div>
  );
