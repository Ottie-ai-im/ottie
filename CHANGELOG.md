...
- Installing the ottie CLI from the macOS desktop app now works in packaged builds.
...
- `additionalModels` option in provider config lets you add or relabel models without replacing the full list — entries merge with runtime-discovered models (ACP) or your static `models` list. See the [Providers docs](https://ottie.app/docs/providers).
- New [Providers docs page](https://ottie.app/docs/providers) covering first-class providers and every custom provider config pattern in one place.
...
- Scripts and services per worktree — define named commands in `ottie.json`, and long-running services get supervised with their own ports and nice proxy URLs like `http://web.my-app.localhost:6767`. See the [worktrees guide](https://ottie.app/docs/worktrees).
...
- `ottie ls` defaults to active agents; pass `-a` to include archived.
...
- Codex keeps fast mode after you approve a plan. ([#526](https://github.com/ottie/ottie/pull/526) by [@therainisme](https://github.com/therainisme))
...
- Markdown files render as formatted markdown in the file pane. ([#427](https://github.com/ottie/ottie/pull/427) by [@aaronflorey](https://github.com/aaronflorey))
- Cmd+L (Ctrl+L on Windows/Linux) focuses the agent message input.
- Provider models refresh on a freshness TTL; Settings shows last-updated time and any fetch errors. ([#426](https://github.com/ottie/ottie/pull/426))
- `disallowedTools` option in provider config to block specific tools from an agent.

### Improved

- Windows: agents launch reliably from npm `.cmd` shims, paths with spaces, and JSON config args — fixes `spawn EINVAL` startup errors. ([#454](https://github.com/ottie/ottie/pull/454))
- OpenCode permission prompts include the requesting tool's context. ([#398](https://github.com/ottie/ottie/pull/398) by [@aaronflorey](https://github.com/aaronflorey))
- OpenCode todo and compaction events render in the timeline. ([#429](https://github.com/ottie/ottie/pull/429) by [@aaronflorey](https://github.com/aaronflorey))
- OpenCode sessions archive cleanly when closed. ([#408](https://github.com/ottie/ottie/pull/408) by [@aaronflorey](https://github.com/aaronflorey))
- OpenCode slash commands recover from SSE timeouts. ([#407](https://github.com/ottie/ottie/pull/407) by [@aaronflorey](https://github.com/aaronflorey))
- Ottie MCP tools work against archived agents, matching the CLI. ([#423](https://github.com/ottie/ottie/pull/423))
- Native scrollbars match the active theme across all web views. ([#399](https://github.com/ottie/ottie/pull/399) by [@ethersh](https://github.com/ethersh))

### Fixed

- Code file previews can be selected and copied on iOS. ([#447](https://github.com/ottie/ottie/pull/447) by [@muzhi1991](https://github.com/muzhi1991))
- File preview no longer shows stale content when reopening the same file. ([#411](https://github.com/ottie/ottie/pull/411) by [@muzhi1991](https://github.com/muzhi1991))
- File explorer reinitialises when the client reconnects after a page refresh. ([#442](https://github.com/ottie/ottie/pull/442) by [@1996fanrui](https://github.com/1996fanrui))
- Generic ACP providers no longer receive duplicated command arguments. ([#444](https://github.com/ottie/ottie/pull/444) by [@edvardchen](https://github.com/edvardchen))
...
- Provider profiles — define custom providers in your Ottie config that appear alongside built-ins. Override a built-in's binary, env, or models, or create entirely new providers. See the [configuration guide](https://github.com/ottie/ottie/blob/main/docs/CUSTOM-PROVIDERS.md).
- ACP agent support — add any ACP-compatible agent to Ottie with `extends: "acp"` in your provider config. No code changes needed.
...
- Ottie tools are no longer injected into agents by default — opt in from Settings when you need agent-to-agent orchestration.
- Agent provider and mode are now resolved server-side, so CLI commands like `ottie run` use consistent defaults without client-side lookups.
...
- Agents get Ottie tools automatically — every new agent gets access to terminals, schedules, worktrees, and other agents through MCP. Toggle it off in Settings under "Inject Ottie tools".
...
- Agent reload — `ottie agent reload` restarts an agent's underlying process from the CLI.
...
- Ottie tool calls in agent timelines show the Ottie logo and human-readable names.
...
- Open in editor — jump from any workspace straight into Cursor, VS Code, Zed, or your file manager. Ottie remembers your choice.
...
- Desktop app now resolves the user's full login shell environment at startup, fixing tools like `codex`, `node`, `bun`, and `direnv` not being found when Ottie is launched from Finder or Dock. Terminals spawned by Ottie now inherit the same PATH and environment variables as a normal terminal session. Approach adapted from VS Code's battle-tested shell environment resolution.
...
- `ottie .` and `ottie <path>` open the desktop app with the given project, similar to `code .`.
...
- Desktop Integrations settings — install the Ottie CLI and orchestration skills directly from the app without touching the terminal.
...
- Setup hint and ottie.app link on the mobile welcome screen so new App Store users know what to do next.
...
- **Terminal management from the CLI** — new `ottie terminal` command group lets you list, create, and interact with workspace terminals without leaving your terminal.
...
- Added `ottie archive` as a top-level alias for `ottie agent archive`.
- Added the `OTTIE_AGENT_ID` environment variable for Claude and Codex agents.
...
- Fixed `ottie send --no-wait` not taking effect.
...
- Improved desktop runtime behavior on Windows by suppressing console windows and defaulting app data to `~/.ottie`.
...
- Update the Ottie desktop app and local daemon directly from Settings.
...
- `ottie wait` now shows a snapshot of recent agent activity while you wait.
