...
buildNpmPackage rec {
  ...
  npmDepsHash = "sha256-0Izp63gVeBO9Efo4Uf8/EB3TkQ9FXJ4Td2Fe5UHm9eE=";
  ...
  installPhase = ''
    ...
    mkdir -p $out/lib/ottie

    # Copy root package metadata
    cp package.json $out/lib/ottie/

    # Copy node_modules (preserving workspace symlinks)
    cp -a node_modules $out/lib/ottie/

    # Auto-detect which @ottie/* packages were built by build:daemon
    # (they'll have a dist/ directory). Copy those and remove the rest.
    for link in $out/lib/ottie/node_modules/@ottie/*; do
      name=$(basename "$link")
      if [ -d "packages/$name/dist" ]; then
        mkdir -p "$out/lib/ottie/packages/$name"
        cp "packages/$name/package.json" "$out/lib/ottie/packages/$name/"
        cp -a "packages/$name/dist" "$out/lib/ottie/packages/$name/"
        if [ -d "packages/$name/node_modules" ]; then
          cp -a "packages/$name/node_modules" "$out/lib/ottie/packages/$name/"
        fi
      else
        rm -f "$link"
      fi
    ...
    # Copy CLI bin entry
    mkdir -p $out/lib/ottie/packages/cli/bin
    cp packages/cli/bin/ottie $out/lib/ottie/packages/cli/bin/

    # Copy extra server files referenced at runtime
    for f in agent-prompt.md .env.example; do
      if [ -f packages/server/$f ]; then
        cp packages/server/$f $out/lib/ottie/packages/server/
      fi
    done

    # Copy server scripts (including supervisor-entrypoint) needed by CLI
    if [ -d packages/server/dist/scripts ]; then
      mkdir -p $out/lib/ottie/packages/server/dist/scripts
      cp -a packages/server/dist/scripts/* $out/lib/ottie/packages/server/dist/scripts/
    fi

    # Create wrapper for the server entry point (for systemd / direct use)
    mkdir -p $out/bin
    makeWrapper ${nodejs}/bin/node $out/bin/ottie-server \
      --add-flags "$out/lib/ottie/packages/server/dist/server/server/index.js" \
      --set NODE_ENV production

    # Create wrapper for the CLI
    makeWrapper ${nodejs}/bin/node $out/bin/ottie \
      --add-flags "$out/lib/ottie/packages/cli/dist/index.js" \
      --set NODE_PATH "$out/lib/ottie/node_modules"
  ...
  meta = {
    ...
    homepage = "https://github.com/ottie/ottie";
